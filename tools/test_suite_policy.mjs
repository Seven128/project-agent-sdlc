...
export const CRITICAL_TEST_SENTINELS = Object.freeze([
  ...
  criticalSentinel(
    ...
    ["long-task", "long-task-trust"],
    "Positive control: [control:static-carrier-pre-run-freeze] and the companion [control:host-derived-target-runtime] reach machine_accepted from package-admitted current Actual. Negative control: canonical [critical:observer-admission-no-bypass] plus R1b and R9 reject custom-Oracle expected-as-actual, Expected-authority carrier substitution and a non-carrier evidence input entering the process runtime closure. Owner/path: createObserverTrustFixture → executeObserverTrustWorkflow traverses Parse/Compile, current snapshot, real runner, package admission/extraction, Counterfactual and Final Gate through long-task-observation-authority.ts, long-task-process-runtime-closure.ts and long-task-admitted-observation.ts. Compile-time attacks bind an owner-specific Compile diagnostic and a real Final-Gate protected-input rejection for the same committed candidate; neither active_task_missing nor the Gate's freshness-first rejection is misreported as an observer-owner finding. Scope: plain exact machine obligations cannot fall through to a named/custom project Oracle or receive Actual from Authority/evidence-role files outside the compiled production closure. Does not prove: arbitrary observer semantics, UI/native/device formats, live-session authenticity or hostile-host isolation.",
  ),
  criticalSentinel(
