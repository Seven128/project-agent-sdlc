...
1. Inventory every material request fragment, attachment, controlling Context unit, canonical schema/specification, selected external constraint, repository-preservation source and delegated instruction with stable identity, digest, disposition and basis. For every marked Material Source Item deterministically derive its paragraphs, list items, table rows, Given/When/Then steps, fenced code blocks and structured configuration lines. Each derived fragment has exactly one `fact_bearing`, `supporting_basis`, `superseded`, `decision_required` or `scope_excluded` disposition; missing, duplicate, conflicting and dangling dispositions block. A `supporting_basis` fragment produces no independent Fact but must name every delivery-semantic Fact it supports; an empty or integrity-only Fact set is not a hiding place. Ordinary Material Source is `non_ui_material` and must contribute Fact lineage. A validated design handoff may classify its independently closed visual facts as `ui_design`; supporting/scope-excluded rows need exact rationale/basis and cannot hide a requirement.
2. Classify every fragment and Fact as `delivery_semantic` or `source_integrity`, and derive its `product`, `technical`, `design`, `acceptance` or `external` authority domain. Digests, file/marker presence and text identity are integrity only; they cannot close delivery meaning. A design/presentation statement cannot exclude or supersede product/technical meaning without an explicit same-domain Source basis. Supersession names both the superseded fragment/anchor and the same-domain superseding Source basis; that successor must itself project the replacement delivery meaning into a same-domain Fact, so a same-domain label or supporting-only note cannot erase prior meaning.
3. Derive and conserve high-signal code spans, API/route paths, versions, enum/symbol names, filenames/schema/config keys, numbers/ranges/units, exact quoted text, modal terms and frozen provider/protocol identifiers. Every anchor maps to a delivery-semantic Fact, explicit same-domain supersession, `decision_required` or `scope_excluded`; integrity-only coverage does not count. Reuse the existing Source inventory and Semantic Fact manifest—never create a second Source ledger or generic NLP registry.
