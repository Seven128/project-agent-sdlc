...
- [ ] Long-Task Anti-Degradation Assurance 已分别检查当前因果链真实性与跨版本非退化：Context 的当前实现描述与 indexed code/runtime/proof owners 一致；设计目的、关键逻辑、两项职责和成立边界未被隐式弱化；若发生替换已有显式 project-owner 决策、old/new implication、非退化影响、兼容/迁移与 replacement proof；防劣化建设保持正净 ROI 且未新增 Authority/Gate/state/matrix/registry。
