...
Each `source_ref` resolves through `authority_sources` to one repository-contained Source document, its current raw-byte digest, one actual marked Source item, the item kind and item-text digest. An arbitrary string or conversation-only locator cannot create authority or enter deterministic recovery. `explicit-user` includes a digest-bound `decision` item. `delegated:<id>` names the stable task-local delegation row; that row carries a digest-bound decision Source ref, bounded scope, allowed origins and allowed target keys, and the accepted Delta binds the same delegation Source ref. Do not substitute an incidental target path for the delegation identity. A declared hierarchical scope such as `checkout.confirmation` covers a target beneath that scope such as `checkout.confirmation.fade_duration`; it never covers a sibling or broader target.
