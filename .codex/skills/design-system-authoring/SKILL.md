...
1. Always read [system-design-reasoning.md](references/system-design-reasoning.md) before generation or a material candidate revision. Create its complete task-local brief before the provider call and pass it through existing `sourceNotes`; the brief is not Authority or hidden chain-of-thought.
...
5. **Reuse or generate candidates.** Reuse an existing provider system only when identity and meaning fit. Otherwise use a live structured creation/revision capability or the official daemon fallback, passing the complete current brief through existing provider `sourceNotes`. In `revise`/`reconcile`, generate bounded deltas against the current closure rather than an unrelated replacement. Keep every output a candidate and retain accepted/rejected/unresolved preference state within its iteration.
6. **Review the handbook and representative scenarios, then iterate.** Inspect candidate `DESIGN.md`, exact Token delta, sparse owner delta, cross-Token/component relationships, the design-system handbook first and supplemental application scenes last. Review target-declared viewport, text-scale, long-label, reduced-motion and other material conditions through the existing reproducible browser/E2E route. Preserve immutable constraints, migration impact, provider diagnostics and provenance. Pending revisions remain non-authoritative, and structural checks never replace aesthetic selection.
