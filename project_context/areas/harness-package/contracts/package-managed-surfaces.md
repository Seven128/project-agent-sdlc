...
- `packages/ty-context/src/lib/design-resource-handoff-*.ts` plus `commands/design-resource.ts` own the provider-neutral embedded-block parser, source-profile/dependency/integrity/addressability/coverage validator and CLI. Long-Task activation calls the same validator and maps verification methods and blocker provenance into existing Contract Claims, Assertions and typed evidence. These are package code, not managed generated assets, a provider runtime, registry, Gate or acceptance surface.
- Package-managed visual-delivery guidance reuses the existing authority split and verification surfaces. The default Workflow performs UI Authority Closure plus a conditional Design Authority Check for material production UI; `context_uiux_design` owns missing/conflicting design-authority repair and deterministic Design Source Projection; durable cross-surface/screen/control facts stay in `project_context/**`, durable visual-system semantics stay in `DESIGN.md`, and exact target facts stay in versioned Source. When a selected handoff exists, `context_development_engineer` conserves its exact covered Source-item/method/blocker/target/condition sets through production components/routes, a cold-start real-user path and independently attributable project checks on the final candidate. This accounting remains task-local and names resource integrity separately from implementation conformance.
