...
The first Rule version accepts only a constant canonical-resource located expected value over a restricted package-owned predicate grammar: enum equality/membership, bounded integer ranges and `all`/`any`/`not`. A deterministic ordered multi-valued decision DAG applies domain-theory partitions, hash-consing, memoized Boolean operations and subtree sharing without ground expansion. Effective Rule/disposition regions must be reachable, mutually exclusive and exactly cover the reachable universe. Complexity fuses bound predicate depth, input nodes, DAG nodes, partition edges and canonical bytes; theoretical ground cardinality is an overflow-safe diagnostic only.

Applicability begins with the complete package-owned atomic property catalog plus validated custom properties. A specified Rule needs matching subject/relation and property Census entries, required Inspector capabilities and closed resource inputs. Every remaining reachable property region needs an exact Source/basis-backed disposition; missing capability, Census, custom-property authority or remainder coverage blocks. Rule identity, semantic proof-obligation identity and non-interference certificate identity are separate typed namespaces. Every property-required method receives its own exact Rule-region obligation.
...
One set-valued certificate may cover multiple Rule identities, omitted axes and dependency edges. Admission separately reports semantic-obligation count, certificate-obligation count, exact covered omitted-axis/dependency-edge sets and counts, canonical DAG nodes/edges/bytes and theoretical cardinality. A declaration, provider `complete` flag or self-consistent dependency list proves no independence. Until a package-owned trusted proof engine establishes non-interference, every omitted axis in a canonical Fact Rule, disposition or population exclusion fails closed and must instead re-enter an explicit exact partition. Canonical work never uses sampling, granularity reduction or expand-then-deduplicate; the structural cost of the conservative partition is Candidate B's independent optimization concern, not permission to weaken Candidate A safety.
...
Long-Task V2 targets project `symbolic_method_bindings` with typed per-Rule/method-region current results and one separately typed `design_symbolic_certificate` binding/result. The existing source-recompiled Final Gate evaluates these alongside ordinary V1 `fact_expectations`/`fact_results` on one current snapshot. No certificate replaces semantic proof, no historical result is reused and no second Authority, Gate, receipt, registry or completion state is introduced.

Purpose-fulfillment efficiency non-degradation is a package mechanism-change admission property, not an AcceptedDeliveryTerminal condition. It requires a named workload, metric, baseline or budget, environment, comparator and tolerance whenever runtime efficiency is claimed. It creates no delivery deadline or fixed execution flow. Non-UI symbolic admission and Provider/P0 trust-boundary work remain out of scope. Either may enter only through a separate evidence-backed admission and owning-Context/Authority decision; the shared symbolic engine alone does not import UI policy into another domain.
...
Long-Task Preflight/Compile requires each Contract `design_target` to match exactly one V1 or V2 target across all declared handoff files by key, representation, interpretation, conditions and complete handoff/resource file set. Every handoff is real `task.source_paths` Source; the owning handoff and every target entry/dependency are Check `verification_inputs`. The adapter preflights, binds and releases one normalized target handoff at a time while retaining only compact identity/digest/error indexes. It preserves deterministic diagnostic precedence—Source-order preflight, duplicate/index errors, Contract-target order, then unbound targets—and does not change the one Contract/Authority/Final Gate semantics.
...
- deterministic input/Contract/runtime mutations: existing `design-resource-handoff*.mjs`/Long-Task suites plus `design-resource-v1-capacity-guard.test.mjs`, `symbolic-denotation-{equivalence,ui-v2,long-task-v2,efficiency-antidegradation,efficiency-guidance}.test.mjs`, the mixed-representation Trust sentinel, `verify_symbolic_denotation_efficiency_delivery.mjs` and distribution/parity checks.
