...
Marker and manifest are an exact current-snapshot pair. Marker plus missing, renamed, empty or invalid manifest is a hard Authority error. A manifest without the marker is also invalid and receives migration guidance to complete the same explicit bundle adoption; it is never treated as a legacy project. A missing or renamed root entry with a retained manifest is an invalid orphan bundle, not `missing` or `legacy`. Outside fenced code, every whole-line HTML comment using the reserved `ty-context-design-authority-format` namespace is first recognized as a machine declaration candidate; only the exact canonical marker bytes above are valid. Internal whitespace, Tab, colon spacing, value trailing whitespace, case variation, an unknown version, an exact marker plus any malformed reserved declaration, a marker outside the fixed position, a duplicate declaration, malformed marked front matter, or physical path casing that differs from its canonical repository-relative spelling fails closed and can never become legacy by lexical omission.
...
## System Design Reasoning And Showcase Projection

Before generating active candidates, `design-system-authoring` records one non-authoritative System Design Reasoning Brief in its existing provider `sourceNotes`. The brief covers audience/tasks, product/platform/environment, accepted/rejected/unresolved direction, desired and avoided character, layout/whitespace/density/focus/accent, typography occupancy, visible versus hit geometry, containers/cards/radius/border/elevation, semantic action/component classes, input/focus/accessibility/motion/responsive behavior, and each external reference as `principle -> project translation -> do not copy`. Every selected choice names its rationale, Token/component consequence, counterexample and intended showcase coverage. Handbook primitives are primary; application scenes are supplemental. Rejected candidates never enter the active authority tree.

An adopted bundle may expose one optional persistent static showcase through this exact entry marker in `DESIGN.md`:

```md
<!-- ty-context-design-showcase path="docs/design-system-showcase/showcase.manifest.json" -->
```

The referenced strict `design-authority-showcase-v1` manifest is repository-local but outside `design_system/**` and outside the Authority closure/digest. It binds the current authority entry, closure digest and diagnostic revision; `status: adopted`; an inert HTML entry and local asset paths with SHA-256; rendered or explicitly N/A coverage for handbook sections; stable Token/component/condition indexes; and `external_network_dependencies: []`. HTML has exactly one closed root containing every non-comment tag and all non-whitespace text, stable literal data/URL attributes and no executable script or network dependency. The deliberately restricted CSS subset rejects comments and escapes that could hide active/network tokens, scans network URL strings as well as `url(...)`, and resolves every local URL to a digest-declared asset. The showcase illustrates current adopted authority but owns no Token, component, rule, Fact, expected value, selection or acceptance meaning and cannot satisfy selected-design or production-conformance proof.

Default inspection reports an optional showcase summary without making it a readiness condition. `design-authority inspect --require-showcase` additionally validates its marker, schema, safe paths, current authority identity, hashes, coverage and inert/no-network constraints. Existing projects without a marker remain valid; a missing or stale showcase fails only the explicit require mode. Human aesthetic suitability remains a user/external judgment.

The active tree holds one current canonical record per stable target. DSA adoption creates a new immutable authority identity and then updates the one current pointer; ordinary implementation-binding refresh keeps the canonical design unchanged. Previous active copies may leave the tree only when current references and active Long-Task Contract bindings are clear and Git/tag/release history preserves audit recovery. Stable files and owners do not gain version suffixes for routine changes.

## Package And Skill Boundary

Package code owns the strict manifest/identity/assessment/showcase codecs, local validation, deterministic digest and Token projection, Doctor diagnostics and read-only CLI inspection. It may not perform remote generation, authentication, network disclosure, polling, retry, cancellation, Provider cleanup, candidate selection or adoption. `@google/design.md` is accessed only through the local `DesignMdToolAdapter` lint/parse/export/diff facade.
