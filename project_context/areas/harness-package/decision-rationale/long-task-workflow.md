...
The selected target must exist as Context-reachable Source or a frozen verification input before fidelity implementation is accepted. Owning Context/`DESIGN.md` connects a stable target key to the readable immutable adopted version/digest, declared conditions and editable upstream owner/locator/update route, and each affected workflow opens the target before deciding. Updating it creates a new immutable version rather than overwriting the adopted baseline. Generated implementation screenshots and diffs remain artifacts and cannot promote themselves into target authority. Browser Claims use `ui_browser`; native/mobile/desktop Control proof uses `runtime_behavior` on its declared product target. A typed `design_conformance` record is distinct from `visual_render`: it binds the selected target identity and declared conditions to current actual/comparison artifacts, while the project verifier remains responsible for the comparison observation. This closes hidden-resource, circular-self-baseline and undeclared-design paths without adding a visual Claim kind, resource registry, new risk level, second Gate, approval state or required design directory.
...
3. existing Contract Claims, Controls/`surface_bindings`, method-specific Assertions, typed evidence and Final Gate own production consumption and acceptance.

The machine closure follows the same causal boundary. Source profiles prove one entry and its exact declared dependency set were completely acquired and hashed. Coverage closes each applicable `subject × target × condition × dimension` cell instead of accepting a broad row. Every declared verification method binds to an independent positive Assertion containing relevant Source-item Claims and the required `design_conformance`, interaction and target-runtime capabilities. Every blocker preserves its Source-item and method lineage into target-local machine proof or a target-blocking External Confirmation. Current-snapshot Final Gate is the only Long-Task acceptance carrier. Authoring/preflight success proves only input completeness and integrity.
