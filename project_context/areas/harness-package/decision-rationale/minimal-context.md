...
The first Router release is intentionally experimental and additive. It scans all eligible `project_context/**` with literal terms under explicit candidate-file, content-byte, term and output budgets, distinguishes registered and unregistered candidates, reports ambiguity and incomplete content scans deterministically and writes nothing. Those Router budgets begin only after the Shared Context Catalog has completed repository-wide Context discovery and metadata collection; 0.10 intentionally keeps that Catalog traversal complete instead of silently truncating Validate/Doctor semantics. Any future total discovery budget must stay Catalog-owned, expose typed incompleteness to every consumer and make Router return `complete: false`/exit `4`; a Router-only walker or silent Validate/Doctor truncation is forbidden. The existing Workflow search remains mandatory and unchanged. Router promotion requires frozen fresh-Agent baseline/candidate pairs proving zero critical-owner omission, no recovery regression, bounded fixed cost and no systematic unregistered loss; static JSON determinism or lower candidate bytes alone cannot authorize replacing the search.
...
Multi-file mutation uses recoverability rather than pretending that ordinary filesystem renames form one physical transaction. The selected owner records original and target bytes, SHA-256, mode, existence/file identity, Catalog identities, deterministic commit order and any target directories known absent; validates a logical overlay; and performs a second cooperative CAS immediately before each publication. Its one logical journal is published as immutable, monotonically numbered same-directory snapshots: every generation binds its predecessor digest, no update replaces an existing journal path, and an older generation is removed only after its successor is durably published. Before a present endpoint is published, the journal records the exact staged temporary identity; after each path succeeds, its next generation records the actual live endpoint identity and mode. A crash may therefore leave one current generation or a validated contiguous suffix, including one exact owned interrupted publication, without making same bytes from a different inode authoritative. Recovery can finish or reverse from any admitted full snapshot or exact staged identity, but never overwrites a same-byte replacement, mode or hard-link change, bytes matching neither side, follows links, removes a non-empty directory or exposes a generic force bypass. This is cooperative CAS with pre-operation recheck, journal publication and post-write readback, not hostile-writer filesystem atomicity. Journal creation/recovery and Long-Task Authority commit reuse the existing Active Authority lock: the mutation side rechecks affected bindings, and first Compile, activation plus in-place Revision refuse an unfinished journal. If an already-active binding conflicts with crash recovery, recovery performs no write; the binding must first end through the legitimate Long-Task lifecycle, then the transaction is completed or rolled back, and only then may a fresh Compile/rebind or required Authority Revision establish the new identity. No force-clear, lock deletion or Revision-while-journal-exists path is an authorization shortcut.
