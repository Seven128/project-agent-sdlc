...
- Area is the durable ownership/discovery unit rather than a required one-to-one workspace model. Consumer projects record useful Area-to-code/workspace ownership in Area or architecture Context with existing fields; `read_policy`, triggers and default selection are retrieval guidance rather than read or edit permissions.
...
- Default multi-Area work separates retrieval from mutation: resolve a genuinely ambiguous sibling product target before edits, enumerate intentional cross-Area targets, treat shared/backend reads as supporting rather than automatic edit scope, and run a project-owned exact changed-path scope verifier when the repository exposes one. Without one, current-diff owner/scope review remains part of Contract/Architecture Conformance.
