...
- Test feedback/cost: `tools/affected_change_discovery.mjs`, `affected_test_selection.mjs`, `test_suite_policy.mjs`, `test_suite_lane_policy.mjs`, `run_required_critical_sentinel.mjs`, package build fingerprinting and package-suite runners/reporters. The single-sentinel runner composes the existing registry and reporter and fails closed unless one platform-applicable sentinel is observed exactly once, attributable to its registered owner file and passing; these tools select regression evidence and never create delivery acceptance.
