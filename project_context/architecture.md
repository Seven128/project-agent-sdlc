...
- Context structural writes are staged and content-identity protected. `context create` writes only an unregistered scaffold and uses exclusive no-replace publication so a concurrently appearing destination is never overwritten. Lossless Manifest patching, current-byte CAS, an owner journal and explicit recovery precede register/move multi-file publication; a single-file primitive is not generalized into a cross-file atomicity claim.
