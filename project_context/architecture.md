...
- Anti-degradation changes are effect-first. A safeguard must close an independent real path, reuse the lightest owner-local mechanism, add no second Authority/Gate/state and show positive total ROI after effect equivalence.
- Static guidance and parity checks establish distribution and known structure, not Agent adherence, provider execution or optimal ROI. Those claims require the existing controlled benchmark/trace boundary.
