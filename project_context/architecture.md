...
- Context manifest/graph/validation/export/sync/doctor: an immutable Shared Context Catalog under `packages/ty-context/src/lib/context-catalog/**` owns parsed/normalized Manifest facts, registered and unregistered discovery, reusable diagnostics and default-footprint projection. Each discovered Context file keeps one NFC public key plus its exact physical repository-relative and absolute spelling, and the Catalog resolver is the only existing-file key-to-filesystem bridge for Catalog consumers; validators, `context-*`, `sync-engine.ts`, `doctor.ts` and managed templates consume those projections through their current public entrypoints. Long-Task Context freezing preserves the same split inside its pre-existing complete protected-input walker: closure keys/topology are NFC and bytes come from the exact discovered physical file. Compile-time semantic-input validation and finalization raw-byte freezing reuse that capture's ephemeral physical map; the map never enters the compiled Contract, Authority snapshot or persisted Context identity.
...
- Context mutation: `packages/ty-context/src/lib/context-create/**` owns the bounded unregistered scaffold write: safe parent traversal, same-directory complete temporary bytes and exclusive no-replace single-file publication. `packages/ty-context/src/lib/context-mutation/**` is the separate future staging/CAS/journal/recovery owner for register and move. It reuses Catalog validation over a candidate overlay and current Long-Task binding/Authority checks; no ordinary command claims a multi-file filesystem transaction is physically atomic.
...
- Test feedback/cost: `tools/affected_change_discovery.mjs`, `affected_test_selection.mjs`, `test_suite_policy.mjs`, `test_suite_selection.mjs`, `test_suite_lane_policy.mjs`, `test_title_inventory.mjs`, `run_required_critical_sentinel.mjs`, package build fingerprinting and package-suite runners/reporters. `test_suite_policy.mjs` remains the sole critical-registry, platform, Trust-membership and reviewed-class owner; `test_suite_selection.mjs` is the sole projection of discovered test files into default, complete Long-Task or Trust population. One AST-derived, non-persistent title inventory validates every selected test root and its transitive test-owned local-module closure against the registry before execution, so every registered critical ID is declared exactly once in its reviewed owner even when platform applicability would exclude its runtime result; unresolved or nonlocal dynamic imports in that test-owned closure fail closed, while production/package modules remain runtime dependencies rather than declaration owners. Both complete package suites and the single-sentinel runner consume that same projection. The single-sentinel runner additionally uses the package-owned contained command runner to load the complete selected root population under the target-ID name pattern with fixed 300-second and output bounds plus required process-tree quiescence, feeds every target-pattern-emitted occurrence to the existing reporter, and succeeds only when that one platform-applicable sentinel is attributable to its registered owner and passing. Its separate `required-critical-sentinel-report-v1` envelope declares the exact verified ID, projection status, incomplete registry runtime observation, unexecuted semantic population and `complete_suite: false`; it is a bounded registration projection, not full-suite acceptance. These tools select regression evidence and never create delivery acceptance, a second registry or workflow state.
...
- Critical runtime declaration binding: the AST inventory exports the unique registered occurrence's file, exact static title, line and column to both runtime report paths. Reporter coverage rejects relative event-file spellings, requires every emitted registered occurrence to equal that declaration identity in addition to registry owner and platform-independent uniqueness, and requires one per-file summary for every selected root. Test-owned dynamic imports must resolve physically under `tests/ty-context`; static production/package imports remain runtime dependencies, but their callable re-exports cannot substitute a different critical call site. Direct or test-owned transitively re-exported module-loader hooks, executed direct/reflected/destructured constructor access, identity-preserving constructor `valueOf()` and role-transfer side effects in loader derivation remain role-bound and fail before hidden registration; statically known non-constructor computed keys and nested constructor data remain ordinary lexical data.
...
- Context structural writes are staged and content-identity protected. `context create` writes only an unregistered scaffold and uses exclusive no-replace publication so a concurrently appearing destination is never overwritten. Register/move use lossless parser-ranged Manifest patches, complete candidate-tree Catalog/reference validation, current-byte/file-identity CAS, synchronized same-directory temporaries, one no-replace journal, deterministic commit order and explicit complete/rollback recovery. Move also journals any missing target-directory chain. These owners provide recoverability and active Long-Task refusal without claiming filesystem-wide multi-file atomicity.
