...
- Test feedback/cost: `tools/affected_change_discovery.mjs`, `affected_test_selection.mjs`, `test_suite_policy.mjs`, `test_suite_selection.mjs`, `test_suite_lane_policy.mjs`, `test_title_inventory.mjs`, `run_required_critical_sentinel.mjs`, package build fingerprinting and package-suite runners/reporters. `test_suite_policy.mjs` remains the sole critical-registry, platform, Trust-membership and reviewed-class owner; `test_suite_selection.mjs` is the sole projection of discovered test files into default, complete Long-Task or Trust population. One AST-derived, non-persistent title inventory proves canonical declarations only within its supported lexical grammar: every registered critical ID has one static title and one reviewed file/title/line/column owner, while unsupported dynamic/module-loader flows fail closed. It does not claim to prove that arbitrary never-executed JavaScript metaprogramming cannot register a test. Both complete package suites and the single-sentinel runner consume that same projection and reporter. The single-sentinel runner loads the selected registration population under a target-ID name pattern with fixed time/output/process-tree bounds, but its `required-critical-sentinel-report-v1` explicitly declares registration projection, incomplete registry observation, unexecuted semantic population and `complete_suite: false`. Every `required_platforms` Long-Task sentinel additionally requires the same unfiltered complete Long-Task population to run on that platform through `run-package-suite`; only that current-platform run can establish actual registration plus semantic runtime closure. These tools select Package-CI regression evidence and never create delivery acceptance, a second registry, reporter, Gate or workflow state.
