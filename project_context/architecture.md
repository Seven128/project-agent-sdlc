...
- Test feedback/cost: `tools/affected_change_discovery.mjs`, `affected_test_selection.mjs`, `test_suite_policy.mjs`, `test_suite_selection.mjs`, `test_suite_lane_policy.mjs`, `test_title_inventory.mjs`, `run_required_critical_sentinel.mjs`, package build fingerprinting and package-suite runners/reporters. `test_suite_policy.mjs` remains the sole critical-registry, platform, Trust-membership and reviewed-class owner; `test_suite_selection.mjs` is the sole projection of discovered test files into default, complete Long-Task or Trust population. One AST-derived, non-persistent title inventory validates that selected population against the registry before execution, so every registered critical ID is declared exactly once in its reviewed owner even when platform applicability would exclude its runtime result; both complete package suites and the single-sentinel runner consume that same projection. The single-sentinel runner then composes the existing registry and reporter and fails closed unless one platform-applicable sentinel is observed exactly once, attributable to its registered owner file and passing. These tools select regression evidence and never create delivery acceptance, a second registry or workflow state.
