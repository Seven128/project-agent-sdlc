...
- Keep three cooperating capabilities distinct: **Minimal Context** preserves durable project facts; the automatically applicable, model-led **default Workflow Contract** is a lightweight engineering protocol for implementation work of any complexity; and the explicitly selected **Single-Goal Long-Task Workflow** adds traceable, recoverable and auditable machine completion authority when that assurance is required.
...
- A formal selected Web/App handoff derives its Expected Fact Universe before generation and is accountable for exact frozen-Inspector/manifest/handoff input equality across every applicable `subject × target × condition × variation × atomic property` Fact Cell and required proof method. Canonical resources remain the exact-value owner. The default route reads every affected selected exact target or constraint, identifies its declared conditions and production owners, runs attributable project-native final-candidate checks and reports unchecked conditions; Long-Task alone projects the exact Fact/method universe into machine acceptance. Input preflight never proves production conformance.
- Material non-UI Source remains authoritative across product/business meaning and technical/backend/architecture meaning. The default route identifies all expressed material requirements, key conditions, failure boundaries and acceptance paths at risk-proportional depth, implements them through the correct owners, uses fresh current-candidate project evidence and reports any unresolved or unverified scope; it does not maintain a complete Fact/obligation ledger or exact-set proof. Long-Task authoring additionally derives an **Expected Semantic Fact Universe** at `subject or relation × atomic applicability × atomic property` granularity and projects stable identities and proof bindings into its existing Contract and Final Gate. Neither route may let current code, aggregate labels, undeclared defaults or unresolved decisions silently redefine Source.
...
- Keep completion claims evidence-bounded. The default route never folds unresolved or unverified scope into an unqualified handoff and reruns affected checks after the last relevant change; Long-Task never substitutes prose, historical evidence or Agent judgment for its current-snapshot machine proof.

## Non-goals / Boundaries

- The default Workflow Contract is the automatically applicable prompt-level execution protocol whenever no Long-Task binding or explicit selection applies. It handles both local and complex work through risk-proportional depth, uses platform-internal planning, creates no required plan/result artifact and has no validator, Receipt, persistent workflow state or machine-completion result.
- Its route is: resolve minimum relevant Context and widen when needed; identify material requirements, key conditions, owners, failure boundaries and real acceptance paths; complete Architecture Deliberation with applicable-quality assessment; decide `Context Delta`; implement precisely under Goal-owned quality guardrails; run fresh project-owned verification; localize and repair failures; perform current-candidate Contract Conformance carrying Engineering Quality Conformance including Architecture Conformance; report implemented, verified, unverified and decision/external-pending scope; then check reverse Context drift.
- The active long-task design remains **Single-Goal Rolling Delivery V2**: one selected delivery, one complete Contract authority, one currently selected host Goal, one verification workspace, one unconditional post-lock host model-change boundary, Goal-owned adaptive implementation and one source-recompiled Final Gate. “One Goal” is not a Harness-persisted Goal ID: compaction may continue inside the current Goal, while a later physical Goal/session reconstructs semantic state with `resume`.
- Long-Task activates only through an explicit selection of the `long-task-workflow` Skill or a valid active binding. The need for durable machine completion authority, cross-compaction/session recovery or audit-grade requirement-to-proof traceability motivates that choice; task duration, complexity and file count neither activate it nor make a small task ineligible. The Skill and its references own Source/Contract authoring, proof, protected revision and lifecycle detail.
- Product Surface/Screen contracts use existing Context roles and do not duplicate visual tokens or target pixels. The default route consumes selected-design authority without reconstructing an exact proof ledger; Long-Task uses only its compiled exact carrier and Final Gate.
