...
- Preserve that protection at the lowest practical total cost. First prove `Coverage_new ⊇ Coverage_old`, `FalseNegative_new ⊆ FalseNegative_old` and non-bypassable Authority/fail-closed/complete-final-snapshot proof; if non-degradation cannot be proved, preserve the current formal acceptance path. Only then compare all incremental Authoring, Runtime, State, Recovery, maintenance, test, process, introduction and migration cost; benefit greater than all incremental cost is positive ROI and permits consideration, not automatic adoption. No cost reduction compensates for weaker drift interception.
