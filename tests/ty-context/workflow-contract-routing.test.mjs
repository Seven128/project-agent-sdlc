...
test("long-task Skill is the only active long-task workflow", async () => {
  ...
  assert.match(
    active,
    /no development phase\/method Gate[\s\S]{0,180}(?:agent )?allocator\/scheduler[\s\S]{0,160}persistent (?:delegation )?state/iu,
  );
  assert.match(
    active,
  ...
  assert.match(
    active,
    /Each worker gets one self-contained packet/iu,
  );
  assert.match(
    active,
    /parent inspects the actual diff[\s\S]{0,120}integrates results/iu,
  );
  assert.match(
    active,
