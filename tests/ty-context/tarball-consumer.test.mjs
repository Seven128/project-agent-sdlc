...
test("actual tarball clean install supports minimal init and real schema4 upgrade without retired runtime",async(t)=>{
  ...
  assert.ok(!entries.some(file=>/^(?:dist\/.*(?:long-task|design-resource|symbolic)|assets\/(?:skills|runtime|templates))/.test(file)));
  ...
  assert.equal(JSON.parse(run(fresh,[cli,"context","list","--default","--json"])).complete,true);
  run(fresh,[cli,"validate-context"]);
  assert.notEqual(spawnSync(process.execPath,[cli,"long-task","help"],{cwd:fresh}).status,0);
