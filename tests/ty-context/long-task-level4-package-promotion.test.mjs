...
test("[critical:level4-package-promotion-boundary] the unique materializer reproduces exact HEAD package bytes and the five-way identity", { timeout: 300_000 }, async () => {
  const temporary = await mkdtemp(path.join(os.tmpdir(), "ty-level4-package-"));
  ...
  try {
    for (const label of ["first", "second"]) {
      ...
      results.push(result);
      await gitText(repositoryRoot, ["worktree", "remove", "--force", checkout]);
    }
    const [first, second] = results.map((item) => item.record);
    assert.equal(first.commit, commit);
    assert.equal(first.tree, await gitText(repositoryRoot, ["rev-parse", `${commit}^{tree}`]));
    ...
    assert.equal(first.package_file_set_sha256, second.package_file_set_sha256);
    ...
    assert.deepEqual(first.command_records.map((item) => item.label), [
      "git-worktree-add", "candidate-head", "candidate-tree", "npm-ci",
      "package-build", "package-check-source", "package-pack", "npm-version",
      "candidate-status",
    ]);
    const packed = readPackedPackageIdentity(results[0].tarball_bytes);
    assert.equal(packed.package_sha256, first.package_sha256);
    assert.equal(packed.package_file_set_sha256, first.package_file_set_sha256);
    reproducedTarball = Buffer.from(results[0].tarball_bytes);
  } finally {
    for (const result of results)
      await gitText(repositoryRoot, ["worktree", "remove", "--force", result.checkout]).catch(() => {});
...
test("materialization, comparator, and promotion reject injected runners or fake comparators", async () => {
  ...
  await assert.rejects(
    () => materializeLongTaskPackage({
      repositoryRoot, commit, checkout: "x", outputDir: "y", commandRunner: async () => {},
    }),
    /long_task_package_materialization_options/u,
  );
  await assert.rejects(
    () => comparePackedPackages({
      repositoryRoot, candidateCommit: commit, promotionCommit: commit,
  ...
  await assert.rejects(
    () => verifyLevel4GovernancePromotion({
      repositoryRoot, promotionCommit: commit, evidenceRoot: repositoryRoot,
...
test("package mutation and formal report package mismatch fail closed", async () => {
  ...
  const evidenceReference = {
    candidate: {
      ...
      package_version: "0.8.14", package_sha256: "c".repeat(64),
    },
  ...
  const report = {
    ...
    capability_level: "level_3", level_4_claimed: false,
    governance_judgment_included: false,
    formal_status: "total_roi_positive", total_roi_supported: true,
    total_roi_positive: true,
    formal_runtime_tcb_identity_sha256: evidenceReference.runtime_tcb_identity_sha256,
    candidate_package: {
  ...
  assert.throws(
    () => parseAndValidateLevel4FormalReport(Buffer.from(JSON.stringify(report)), evidenceReference),
...
test("manifest v2 is exact and legacy, mixed, or unknown-newer manifest families fail closed", () => {
  const entries = [{ path: "setup/c/candidate.tgz", role: "package_tarball", bytes: 1, sha256: "a".repeat(64) }];
  const manifest = {
    schema_version: REAL_PROCESS_SCHEMAS.REAL_PROCESS_MANIFEST_SCHEMA,
    root: ".", excludes: ["attestation.json", "manifest.json"], entries,
    entry_count: 1, total_bytes: 1,
    materialized_set_sha256: sha256(canonical(entries)),
  };
  assert.deepEqual(parseAndValidateLevel4RunSetManifest(Buffer.from(JSON.stringify(manifest))), manifest);
  for (const schema of ["long-task-real-process-roi-manifest-v1", "long-task-real-process-roi-manifest-v999"])
    assert.throws(
      () => parseAndValidateLevel4RunSetManifest(Buffer.from(JSON.stringify({ ...manifest, schema_version: schema }))),
...
async function gitText(cwd, args) {
  const result = await execFileAsync("git", args, {
    cwd, windowsHide: true, encoding: "utf8", timeout: 120_000,
