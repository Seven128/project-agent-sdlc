...
test("mechanism benchmark fixes baseline, tracks, tasks, gold, and hidden boundaries", async () => {
  const experiments = JSON.parse(
    await readFile(path.join(mechanismRoot, "experiment-set.json"), "utf8"),
  );
  assert.equal(
    experiments.baseline_commit,
    "c030d02eee315d2860c6a2ff01c22887690f3684",
  );
  assert.deepEqual(Object.keys(experiments.tracks).sort(), [
    "context-routing",
    "long-task-authoring",
    "workflow-assurance",
    "workflow-expression",
  ...
  const taskIds = [
    ...new Set(
      Object.values(experiments.tracks).flatMap((track) => track.tasks),
    ),
  ];
  assert.equal(taskIds.length, 12);
  for (const id of taskIds) {
    await assert.doesNotReject(
      readFile(path.join(mechanismRoot, "tasks", `${id}.json`), "utf8"),
    );
    await assert.doesNotReject(
      readFile(path.join(mechanismRoot, "gold", `${id}.json`), "utf8"),
    );
  }
  const serialized = JSON.stringify(experiments);
  assert.doesNotMatch(
    serialized,
    /benchmark-proven|completed result|speedup achieved/iu,
  );
  assert.deepEqual(experiments.tracks["workflow-assurance"].variants, [
    "workflow-exact-ephemeral-baseline",
    "workflow-assurance-split",
  ]);
  assert.equal(
    experiments.tracks["workflow-assurance"].pair_policy.minimum_pairs,
    3,
  );
  assert.equal(
    experiments.tracks["workflow-assurance"].pair_policy
      .high_variance_or_near_threshold_pairs,
    5,
  );
  assert.equal(
    experiments.tracks["workflow-assurance"].decision_thresholds
      .false_complete_free_rate,
    1,
  );
  const assuranceSources = experiments.tracks[
    "workflow-assurance"
  ].variants.map((id) => experiments.variants[id].guidance_source);
  assert.deepEqual(
    assuranceSources.map(({ commit, path }) => ({ commit, path })),
    [
      {
  ...
  for (const source of assuranceSources) {
    ...
    assert.equal(
      source.measured_section.start_heading,
...
test("workflow assurance variants freeze the old exact default and the new model-led boundary", async () => {
  const temp = await mkdtemp(
    path.join(os.tmpdir(), "mechanism-assurance-guidance-"),
  );
  try {
    const common = {
      task: "local-rounding-bug",
      pairId: "assurance-guidance",
      replicate: 1,
      model: "fixed-model",
    ...
    await prepareMechanismRun({
      ...common,
      variant: "workflow-exact-ephemeral-baseline",
      outDir: baselineDir,
    });
    await prepareMechanismRun({
      ...common,
    ...
    const baseline = await readFile(
      path.join(baselineDir, "AGENTS.md"),
      "utf8",
    );
    const candidate = await readFile(
      path.join(candidateDir, "AGENTS.md"),
      "utf8",
    );
    const baselineMetadata = JSON.parse(
      await readFile(
        path.join(baselineDir, ".benchmark", "mechanism-run.json"),
        "utf8",
      ),
    );
    const candidateMetadata = JSON.parse(
      await readFile(
        path.join(candidateDir, ".benchmark", "mechanism-run.json"),
        "utf8",
      ),
    );
    assert.match(
      baseline,
      /Expected Semantic Facts = Source Indexed Facts = implementation\/acceptance accounted Facts/u,
    );
    assert.match(
      baseline,
      /Fact × required-method obligations = attributable current-candidate result rows/u,
    );
    assert.doesNotMatch(
      candidate,
      /Expected Semantic Facts = Source Indexed Facts/u,
    );
    assert.match(
      candidate,
      /model-led default Workflow Contract for work of any complexity/u,
    );
    assert.match(candidate, /Perform evidence-bounded Contract Conformance/u);
    assert.match(
      candidate,
      /Report `Implemented`, `Verified`, `Unverified`, `Blocked \/ decision required`/u,
    );
    assert.equal(
      baselineMetadata.workflow_instruction_bytes,
      workflowInstructionBytes(baseline),
    );
    assert.equal(
      candidateMetadata.workflow_instruction_bytes,
      workflowInstructionBytes(candidate),
    );
    const experiments = JSON.parse(
      await readFile(path.join(mechanismRoot, "experiment-set.json"), "utf8"),
    );
    assert.deepEqual(
      baselineMetadata.workflow_guidance_source,
      expectedGuidanceProvenance(
        experiments.variants["workflow-exact-ephemeral-baseline"]
          .guidance_source,
      ),
    );
    assert.deepEqual(
      candidateMetadata.workflow_guidance_source,
      expectedGuidanceProvenance(
        experiments.variants["workflow-assurance-split"].guidance_source,
      ),
    );
    assert.notEqual(
      baselineMetadata.workflow_guidance_source.commit,
      candidateMetadata.workflow_guidance_source.commit,
    );
    const guidance = await readFile(
      path.join(mechanismRoot, "runner", "guidance.mjs"),
      "utf8",
    );
...
test("small high-assurance work selects Long-Task without activating it", async () => {
  const temp = await mkdtemp(
    path.join(os.tmpdir(), "mechanism-assurance-route-"),
  );
  try {
    const task = JSON.parse(
      await readFile(
        path.join(mechanismRoot, "tasks", "small-high-assurance-route.json"),
        "utf8",
      ),
    );
    assert.doesNotMatch(
      task.prompt,
      /Task size must not decide the route|proof-floor choice/iu,
    );
    assert.match(task.prompt, /recoverable in a later session/iu);
    assert.match(
      task.prompt,
      /independently locatable and rerunnable acceptance check/iu,
    );
    ...
    const gold = JSON.parse(
      await readFile(
        path.join(mechanismRoot, "gold", "small-high-assurance-route.json"),
        "utf8",
      ),
    );
    await writeFile(
      path.join(runDir, ".benchmark", "agent-result.json"),
      `${JSON.stringify(
        {
    ...
    await writeFile(
      path.join(runDir, ".benchmark", "observer-state.json"),
      `${JSON.stringify({ duration_ms: 1200 })}\n`,
    );
    ...
    await assert.rejects(
      readFile(path.join(runDir, "delivery-contract.yaml"), "utf8"),
    );
  } finally {
    await rm(temp, { recursive: true, force: true });
  }
...
test("workflow assurance aggregation expands from three to five pairs only under the frozen cost rule", async () => {
  const temp = await mkdtemp(
    path.join(os.tmpdir(), "mechanism-assurance-pairs-"),
  );
  try {
    const identity = {
      model: "fixed-model",
    ...
    assert.match(
      expanded.pair_requirement_reason,
      /near:total_tool_call_reduction/u,
    );
  } finally {
    await rm(temp, { recursive: true, force: true });
  }
...
test("stateless Context resolver reaches fixed controlling Context without creating state", async () => {
  ...
  for (const required of [
    ...
    assert.ok(
      [...result.required, ...result.candidates].includes(required),
      required,
    );
  ...
  assert.ok(
    !result.candidates.includes(
      "project_context/areas/invoice-ops/archive/legacy-tax.md",
    ),
  );
});
...
test("UIUX recovery benchmark preserves selected-source closure and one canonical target owner", async () => {
  ...
  for (const track of ["context-routing", "workflow-expression"]) {
    assert.ok(
      experiments.tracks[track].tasks.includes("uiux-selected-target-recovery"),
      track,
    );
  }
  ...
  const task = JSON.parse(
    await readFile(
      path.join(mechanismRoot, "tasks", "uiux-selected-target-recovery.json"),
      "utf8",
    ),
  );
  const gold = JSON.parse(
    await readFile(
      path.join(mechanismRoot, "gold", "uiux-selected-target-recovery.json"),
      "utf8",
    ),
  );
  const handoff = await readFile(
    path.join(fixture, "design", "handoffs", "invoice-board.md"),
    "utf8",
  );
  for (const dimension of [
    ...
    assert.match(
      handoff,
      new RegExp(`dimension:\\s+${dimension}\\b`, "u"),
      dimension,
    );
    assert.match(
      handoff,
      new RegExp(
        `dimension:\\s+${dimension}\\b[\\s\\S]*?condition_refs:\\s*\\r?\\n\\s*- desktop-default\\s*\\r?\\n\\s*- desktop-reduced`,
        "u",
      ),
  ...
  assert.match(
    handoff,
    /kind:\s+implementation_web[\s\S]*acquisition:\s+complete/u,
  );
  ...
  assert.match(
    screen,
    /invoice-board-desktop-v1[\s\S]*sha256:f285fac663d8d08f1d201918a6ce7ebaebc417d80e77bd483f25dc397f24ef98/u,
  );
  assert.match(
    screen,
    /od:\/\/projects\/invoice-operations-lab\/invoice-board/u,
  );
  assert.match(screen, /desktop Web, 1440 × 900, light/u);
  assert.match(
    design,
    /invoice-board-desktop-v1[\s\S]*canonical owner:[\s\S]*invoice-board\.md#design-target-references/u,
  );
  assert.doesNotMatch(
    design,
    /f285fac663d8d08f1d201918a6ce7ebaebc417d80e77bd483f25dc397f24ef98|od:\/\/projects\/invoice-operations-lab\/invoice-board/u,
  );
  ...
  assert.ok(
    routedFiles.includes(
      "project_context/areas/invoice-ops/screens/invoice-board.md",
    ),
  );
  ...
  try {
    ...
    const unsafe = await contextMetrics(
      fixture,
      gold,
      {
        context_files_read: gold.controlling_context,
    ...
    assert.equal(
      unsafe.selection_source,
      "invalid_host_trace_fallback_to_agent_report",
    );
  ...
  const hidden = await readFile(
    path.join(mechanismRoot, "hidden", task.probe),
    "utf8",
  );
  for (let index = 1; index <= 7; index += 1) {
    assert.match(
      hidden,
      new RegExp(`UIUX-${String(index).padStart(3, "0")}`, "u"),
    );
  }
  assert.match(hidden, /event\.command === "design-resource preflight"/u);
  const prepare = await readFile(
    path.join(mechanismRoot, "runner", "prepare.mjs"),
    "utf8",
  );
  assert.match(
    prepare,
...
test("prepare keeps hidden/gold outside the run and records strict pair identity", async () => {
  ...
  try {
    ...
    assert.match(
      await readFile(result.prompt, "utf8"),
      /Do not inspect.*gold files.*hidden probes/iu,
    );
    await assert.rejects(
      readFile(path.join(runDir, "gold", "local-rounding-bug.json"), "utf8"),
    );
    await assert.rejects(
      readFile(path.join(runDir, "hidden", "local-rounding-bug.mjs"), "utf8"),
    );
    assert.match(
      await readFile(path.join(runDir, "AGENTS.md"), "utf8"),
      /Context Resolve R0/,
    );
    await assert.doesNotReject(
      readFile(path.join(runDir, "tools", "context-resolve-r0.mjs"), "utf8"),
    );
    await assert.rejects(
      prepareMechanismRun({
        task: "local-rounding-bug",
...
test("score and compare fail closed until quality, Context, and traced reads are present", async () => {
  ...
  try {
    ...
    for (const runDir of [baselineDir, candidateDir]) {
      await writeFile(
        path.join(runDir, "src", "billing", "money.mjs"),
      ...
      const gold = JSON.parse(
        await readFile(
          path.join(mechanismRoot, "gold", "local-rounding-bug.json"),
          "utf8",
        ),
      );
      await writeFile(
        ...
        `${JSON.stringify(
          {
            ...
            completion_status: "complete",
            implemented_scope: [
              "rounding implementation and regression coverage",
            ],
    ...
    const calibrationBaseline = await scoreMechanismRun({
      runDir: baselineDir,
    });
    const calibrationCandidate = await scoreMechanismRun({
      runDir: candidateDir,
    });
    assert.equal(calibrationBaseline.metrics.hard_gate_passed, true);
    const b0 = path.join(temp, "b0.json");
    const c0 = path.join(temp, "c0.json");
    await writeFile(b0, `${JSON.stringify(calibrationBaseline)}\n`);
    await writeFile(c0, `${JSON.stringify(calibrationCandidate)}\n`);
    const calibrationPair = await compareMechanismScores({
      baselineScore: b0,
      candidateScore: c0,
    });
    ...
    await writeFile(
      invalidTrace,
      `${JSON.stringify({ context_files_read: ["project_context/global.md"], context_read_rounds: 1 })}\n`,
    );
    const invalidTraceBaseline = await scoreMechanismRun({
      runDir: baselineDir,
      trace: invalidTrace,
    });
    const invalidTraceCandidate = await scoreMechanismRun({
      runDir: candidateDir,
      trace: invalidTrace,
    });
    ...
    const invalidB = path.join(temp, "invalid-b.json");
    const invalidC = path.join(temp, "invalid-c.json");
    ...
    const invalidTracePair = await compareMechanismScores({
      baselineScore: invalidB,
      candidateScore: invalidC,
    });
    ...
    await writeFile(
      trace,
      `${JSON.stringify({ schema_version: "tiny-context-host-trace-v1", source: "host_tool_trace", context_files_read: gold.controlling_context, context_read_rounds: 2 })}\n`,
    );
    ...
    const b1 = path.join(temp, "b1.json");
    const c1 = path.join(temp, "c1.json");
    await writeFile(b1, `${JSON.stringify(baseline)}\n`);
    await writeFile(c1, `${JSON.stringify(candidate)}\n`);
    const eligiblePair = await compareMechanismScores({
      baselineScore: b1,
      candidateScore: c1,
    });
    assert.equal(eligiblePair.decision_eligible, true);
    const pairFile = path.join(temp, "pair.json");
    await writeFile(pairFile, `${JSON.stringify(eligiblePair)}\n`);
    ...
    await assert.rejects(
      aggregateComparisons({ scores: [pairFile, pairFile, pairFile] }),
      /distinct paired runs/iu,
    );

    const pair2 = structuredClone(eligiblePair);
    pair2.replicate = 2;
    const pair3 = structuredClone(eligiblePair);
    pair3.replicate = 3;
    const pair2File = path.join(temp, "pair-2.json");
    const pair3File = path.join(temp, "pair-3.json");
    await writeFile(pair2File, `${JSON.stringify(pair2)}\n`);
    await writeFile(pair3File, `${JSON.stringify(pair3)}\n`);
    const threePairs = await aggregateComparisons({
      scores: [pairFile, pair2File, pair3File],
    });
    ...
    await writeFile(pair3File, `${JSON.stringify(pair3)}\n`);
    await assert.rejects(
      aggregateComparisons({ scores: [pairFile, pair2File, pair3File] }),
      /share fixed model, reasoning/iu,
    );
  } finally {
    await rm(temp, { recursive: true, force: true });
  }
...
test("Context Delta scoring rejects touch-only updates and checks durable semantics", async () => {
  const temp = await mkdtemp(
    path.join(os.tmpdir(), "mechanism-context-delta-"),
  );
  try {
    const runDir = path.join(temp, "run");
    await prepareMechanismRun({
    ...
    const gold = JSON.parse(
      await readFile(
        path.join(mechanismRoot, "gold", "cross-module-receipt-event.json"),
        "utf8",
      ),
    );
    ...
    assert.ok(
      touchOnly.content_findings.some(
        (item) => item.forbidden_terms_present.length > 0,
      ),
    );
    ...
    await writeFile(
      path.join(runDir, "project_context", "areas", "notifications.md"),
...
test("formal prepare overlays the fixed Context after Harness init and exposes a local CLI wrapper", async () => {
  ...
  try {
    ...
    await assert.rejects(
      readFile(
        path.join(runDir, "project_context", "areas", "main.md"),
        "utf8",
      ),
    );
    await assert.doesNotReject(
      readFile(
        path.join(runDir, "project_context", "areas", "invoice-ops.md"),
        "utf8",
      ),
    );
    await assert.doesNotReject(
      readFile(
        path.join(runDir, ".codex", "skills", "long-task-workflow", "SKILL.md"),
        "utf8",
      ),
    );
    assert.ok(
      (
        await readFile(path.join(runDir, "tools", "ty-context.mjs"), "utf8")
      ).includes(JSON.stringify(fakeCli)),
    );
    assert.match(
      await readFile(path.join(runDir, ".benchmark", "prompt.md"), "utf8"),
      /node tools\/ty-context\.mjs/,
    );
  } finally {
    await rm(temp, { recursive: true, force: true });
  }
