...
test("design-system workflow separates provider output, selection, adoption and binding", async () => {
  ...
  for (const expected of [
    ...
    /System Design Reasoning Brief[\s\S]*supported input/iu,
  ...
  assert.match(
    reasoning,
    /candidate input, not private chain-of-thought, Design Authority, Context, workflow state or an adoption record/iu,
  );
  assert.match(
    reasoning,
    /accepted preferences, rejected preferences and unresolved choices as three separate sets/iu,
  );
  assert.match(
    reasoning,
    /screen\/layout whitespace[\s\S]*inter-group rhythm[\s\S]*component-internal whitespace[\s\S]*visual-weight whitespace/iu,
  );
  assert.match(
    reasoning,
    /visible surface versus hit target[\s\S]*internal padding versus type line height[\s\S]*group gap versus item gap/iu,
  );
  assert.match(
    reasoning,
    /classified first by user intent, information responsibility and state model, then by appearance/iu,
  );
  assert.match(
    reasoning,
    /source principle, project-specific translation and prohibited copying/iu,
  );
  assert.match(
    reasoning,
    /design goal[\s\S]*scenario-linked rationale[\s\S]*Token, component, relationship or pattern realization[\s\S]*counterexample[\s\S]*showcase scenario/iu,
  );
  assert.match(
    reasoning,
    /primary artifact is an engineering design-system handbook[\s\S]*Application scenes are last[\s\S]*supplemental-validation/iu,
  );
  assert.match(
    reasoning,
    /target-declared viewport, text-scale, long-label and reduced-motion[\s\S]*existing reproducible browser\/E2E route[\s\S]*do not introduce a global fixed mobile viewport policy/iu,
  );
  ...
  assert.match(
    showcase,
    /human-readable handbook[\s\S]*without creating another Design Authority, Context owner, selected-design Fact source, implementation-conformance proof or aesthetic verdict/iu,
  );
  assert.match(
    showcase,
    /design-authority-showcase-v1[\s\S]*artifact_category[\s\S]*design_system_handbook/iu,
  );
  assert.match(
    showcase,
    /identity[\s\S]*color[\s\S]*typography[\s\S]*supplemental-validation/iu,
  );
  assert.match(
    showcase,
    /data-ty-showcase-section[\s\S]*data-ty-showcase-token-family[\s\S]*data-ty-showcase-component[\s\S]*data-ty-showcase-target/iu,
  );
  assert.match(
    showcase,
    /Target-condition scenes are descendants of the final supplemental section[\s\S]*never declares a Token family or component/iu,
  );
  assert.match(
    showcase,
    /Do not use scripts[\s\S]*CSS imports[\s\S]*undeclared\/unreachable assets/iu,
  );
  assert.match(
    showcase,
    /existing reproducible browser\/E2E route[\s\S]*declared viewport, text-scale, long-label, reduced-motion/iu,
  );
  assert.match(
    showcase,
    /valid sidecar proves integrity\/category\/coverage only[\s\S]*selection proves suitability/iu,
  );
  ...
  assert.match(
    provider,
    /complete task-local System Design Reasoning Brief[\s\S]*selected live route actually supports/iu,
  );
  assert.match(
    provider,
    /draft route exposes `sourceNotes`[\s\S]*Field names from an older compatibility snapshot are never assumed/iu,
  );
  assert.match(
    provider,
    /engineering design-system handbook as the primary artifact[\s\S]*Product-page galleries are supplemental validation only/iu,
  );
  ...
  assert.match(
    adoption,
    /design-authority-showcase-v1[\s\S]*artifact_category: design_system_handbook[\s\S]*status: adopted/iu,
  );
  assert.match(
    adoption,
    /outside `design_system\/\*\*` and outside the Authority closure/iu,
  );
  assert.match(
    adoption,
    /application scenes occur last under `supplemental-validation`[\s\S]*cannot declare a second component system/iu,
  );
  assert.match(adoption, /design-authority inspect --require-showcase/iu);
  assert.match(
    adoption,
    /existing reproducible browser\/E2E route[\s\S]*declared viewport, text-scale, long-label, reduced-motion[\s\S]*do not add a global fixed viewport list/iu,
  );
  assert.match(
    provider,
