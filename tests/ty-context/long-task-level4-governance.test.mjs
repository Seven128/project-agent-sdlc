...
import { buildLevel4GovernanceRecords, digestEntry } from "./helpers/long-task-level4-governance-fixture.mjs";
...
test("[critical:level4-governance-audit-boundary] audit binds independence, complete input census, current commands/results, findings, and sole formal owner", () => {
  ...
  const { evidenceReference, auditRecord, ownerDecision, promotionRecord } = fixture;
  assert.equal(validateLevel4EvidenceReference(evidenceReference), evidenceReference);
  assert.equal(validateLevel4IndependentAuditRecord(auditRecord, evidenceReference), auditRecord);
  assert.equal(validateLevel4OwnerDecision(ownerDecision, evidenceReference, auditRecord), ownerDecision);
  assert.equal(validateLevel4PromotionRecord(promotionRecord, evidenceReference, auditRecord, ownerDecision), promotionRecord);
  assert.equal(auditRecord.formal_conclusion_owner, "verify_long_task_real_process_roi");
  assert.equal(auditRecord.formal_roi_conclusion_owned, false);
  assert.deepEqual(auditRecord.inputs.map((entry) => entry.role).sort(), [...LEVEL4_AUDIT_REQUIRED_INPUT_ROLES].sort());
});
...
test("dependent auditors, incomplete or altered inputs, stale commands, and hidden findings fail closed", () => {
  ...
  assert.throws(() => validateLevel4IndependentAuditRecord(dependent, evidenceReference), /level4_auditor_independence/u);
  const participant = structuredClone(auditRecord);
  participant.auditor.collection_participation = true;
  assert.throws(() => validateLevel4IndependentAuditRecord(participant, evidenceReference), /level4_auditor_independence/u);
  const missing = structuredClone(auditRecord);
  missing.inputs.pop();
  missing.input_census_identity_sha256 = "0".repeat(64);
  assert.throws(() => validateLevel4IndependentAuditRecord(missing, evidenceReference), /level4_audit_input/u);
  const failed = structuredClone(auditRecord);
  failed.commands.find((item) => item.command_id === "formal-verifier").exit_code = 1;
  assert.throws(() => validateLevel4IndependentAuditRecord(failed, evidenceReference), /level4_audit_current_results/u);
  const hidden = structuredClone(auditRecord);
  hidden.findings.push({
    finding_id: "open-critical", severity: "critical-counterexample",
    status: "open", summary: "False-acceptance path remains open.",
    input_refs: [hidden.inputs[0].id], command_refs: ["validation"],
  });
  assert.throws(() => validateLevel4IndependentAuditRecord(hidden, evidenceReference), /level4_audit_conclusion/u);
  const secondVerifier = structuredClone(auditRecord);
  secondVerifier.formal_roi_conclusion_owned = true;
  assert.throws(() => validateLevel4IndependentAuditRecord(secondVerifier, evidenceReference), /level4_audit/u);
});

test("owner approval and promotion records cannot drift package, benchmark, runtime, audit, or candidate identities", () => {
  const { evidenceReference, auditRecord, ownerDecision, promotionRecord } = governanceFixture({
    commit: "a".repeat(40), tree: "b".repeat(40),
  });
  const wrongOwner = structuredClone(ownerDecision);
  wrongOwner.audit_record_sha256 = "0".repeat(64);
  assert.throws(() => validateLevel4OwnerDecision(wrongOwner, evidenceReference, auditRecord), /level4_owner_decision/u);
  for (const field of [
    "package_sha256",
    ...
    assert.throws(
      () => validateLevel4PromotionRecord(drift, evidenceReference, auditRecord, ownerDecision),
...
test("direct-child governance-only additions pass the commit boundary, while any candidate/TCB-surface mutation fails before evidence use", async () => {
  const temporary = await mkdtemp(path.join(os.tmpdir(), "ty-level4-governance-"));
  ...
  try {
    ...
    await assert.rejects(
      () => verifyLevel4GovernancePromotion({ repositoryRoot: repository, promotionCommit: promotion, evidenceRoot }),
      /ENOENT|formal_evidence_regular_file/u,
    );

    await git(repository, ["switch", "--detach", commit]);
    await writeGovernanceRecords(repository, governanceRoot, records);
    await writeArtifact(repository, "PROJECT_SPEC.md", "forbidden promotion mutation\n");
    await git(repository, ["add", "."]);
    await git(repository, ["commit", "-m", "invalid promotion mutation"]);
    const invalid = await git(repository, ["rev-parse", "HEAD"]);
    await assert.rejects(
      () => verifyLevel4GovernancePromotion({ repositoryRoot: repository, promotionCommit: invalid, evidenceRoot }),
...
function governanceFixture({ commit, tree }) {
  const candidate = {
    commit, tree, package_version: "0.8.14", package_sha256: "c".repeat(64),
  };
  ...
  ].map(([id, role, locator]) => digestEntry(id, role, locator, Buffer.from(`${role}\n`)));
  const auditInputs = LEVEL4_AUDIT_REQUIRED_INPUT_ROLES.map((role) =>
    digestEntry(`audit-${role}`, role, `audit/${role}.bin`, Buffer.from(`${role}\n`)),
  );
