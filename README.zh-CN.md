...
Compile 为每个机器 Claim 或 Fact × required-method obligation 派生内部 `CompiledObservationAuthority`，它不是新的 Contract Authority、状态或 registry。首版只有两条机器路径：`package_static_json_exact` 读取 runner 前快照中已存在、runner 后 no-follow 文件 identity/digest 不变、匹配生产 Binding 且不属于 Source/Context/Contract/expected/evidence/report/status/verifier output 的 UTF-8 JSON 静态结构；它只证明静态内容，不证明运行时消费。`package_process_json_exact` 只支持 Source-backed 的 `runtime_family: process`、`role: product` 目标，以及 target 和完整 argv 与该权威一致的直接 root `project_binary`。每个 required target 都由一项 canonical Source technical obligation 精确覆盖 key、role、family、root、完整 argv 和 capabilities。Compile 只把该 invocation、production owner/Binding 覆盖的 argv 仓库文件、以及当前 Claim/Counterfactual 的 production carrier 放进内部 process runtime closure；`input_paths` 继续描述 Check scope/freshness，但不会自动进入快照。closure 中任何文件都不得兼任 Source/Context/Contract/canonical expected、verification input、expected output/artifact、evidence/status/report/comparison/Receipt/Long-Task workdir 或历史 session/evidence。Harness 仅复制这份 closure，并把其 identity 绑定到 host attestation。child 只得到最小环境且在 stdout 输出一个受限的 `ty-context-product-observation-v1` envelope；一次输出可承载多个 Fact。无法显式 production-bind 的依赖或无法直接绑定 root 输出面的 Claim 必须 External Confirmation。项目 payload 继续使用 v3，不发布 v4，也不建设通用 UI/native observer 或语言依赖解析器。
...
Contract 声明一个有界 target profile、非空 required product target refs，以及每个 target 的 runtime family、root entrypoint、完整 root argv 和显式 capabilities。每个 required target 还必须通过 Source Claim disposition 映射到一项相同 identity 的 canonical Source technical obligation；process root/argv 文件同时属于 production owner 与 Binding。Web/process 代理不能代替单独要求的 Native/desktop 目标。当前机器 `target_runtime` 只有 Harness 直接启动该 Source-backed process product root 才能成立；browser/native/desktop/device 要求保留阻断性 External Confirmation。每个 `critical_user_path` Outcome 和 Stage gate 都必须通过 admitted root proof 或对应 External Confirmation 处理每个 required target。
