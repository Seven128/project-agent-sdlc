...
材料性的 DRA 修订循环只从绑定 raw-byte digest 的 Base 与完整有序 Delta 语义重放；`origin`、限定的 `decision_authority`、真实且绑定 digest 的 Source item、资源 `evidence_refs` 和 accepted/rejected/unresolved 裁决保持独立，因此资源被选中或任意字符串 ref 都不能授权建议意义。Resource decision 为每个 requirement key 精确绑定 Delta/origin/authority/Source；changed/unchanged/resource-decision/blast-radius 全集均完整 fail closed，且每个 accepted decision 恰有一个 Proposal owner 或精确视觉 resource owner。只有真实跨中断需求才可显式 `create` 一个 ignored、任务局部、非权威 checkpoint；`update` 只能通过调用方给出的 checkpoint digest CAS 替换，`inspect`/`preview` 重新派生当前状态。`apply` 通过写前/写后 raw-byte CAS 与新鲜的 Requirements→Resource、Resource→Requirements、Unexpected Blast Radius 审计写回。`remove` 仅在目录清单证明其中只有 digest 匹配的 helper checkpoint 时完整删除；否则返回 `partial` 并保留非 helper 内容。简单 preview 不创建 checkpoint、持久字节、暂停、Provider run、正式 handoff、Proposal 写回或 helper transaction。checkpoint 与 reconciliation 只是上游恢复/诊断数据，绝不是 Design Authority、Long-Task Source/Evidence 或完成证明。
