...
Compile 为每个机器 Claim 或 Fact × required-method obligation 派生内部 `CompiledObservationAuthority`，它不是新的 Contract Authority、状态或 registry。首版只有两条机器路径：`package_static_json_exact` 读取 runner 前快照中已存在、runner 后 no-follow 文件 identity/digest 不变、匹配生产 Binding 且不属于 Source/Context/Contract/expected/evidence/report/status/verifier output 的 UTF-8 JSON 静态结构；prepare-all mutation observation 与逐文件 pre/post identity/hash 共同拒绝 transient/persistent swap，它只证明静态内容，不证明运行时消费。`package_process_json_exact` 只支持 Source-backed 的 `runtime_family: process`、`role: product` 目标，以及 target 和完整 argv 与该权威一致的直接 root `project_binary`。每个 required target 都由一项 canonical Source technical obligation 精确覆盖 key、role、family、root、完整 argv 和 capabilities。Compile 只把 production Binding 覆盖的 exact root/argv 仓库文件与当前 Claim/Counterfactual exact production carrier 放进声明稳定的 process runtime closure；`input_paths` 和 manifest/glob sibling 都不会自动进入快照。Exact planned member 可以到 Final Gate 才 materialize，未绑定且缺失的 repository-file argv token 在 Compile 失败。closure 中任何文件都不得兼任 Source/Context/Contract/canonical expected、verification input、expected output/artifact、evidence/status/report/comparison/Receipt/Long-Task workdir 或历史 session/evidence。Harness 仅复制这份 closure，并把其 identity 绑定到 host attestation。兼容的 Cross-Check 与 implicit-preserved Facts 共享一个受限 stdout `ty-context-product-observation-v1` envelope，同时保留独立 result identity。无法显式 production-bind 的嵌入式依赖或无法直接绑定 root 输出面的 Claim 必须 External Confirmation。项目 payload 继续使用 v3，不发布 v4，也不建设通用 UI/native observer 或语言依赖解析器。
...
Contract 声明一个有界 target profile、非空 required product target refs，以及每个 target 的 runtime family、root entrypoint、完整 root argv 和显式 capabilities。每个 required target 还必须通过 Source Claim disposition 映射到一项相同 identity 的 canonical Source technical obligation；process root/argv 文件同时属于 production owner 与 exact production Binding。Compile 只从 exact root/argv/carrier 路径生成声明稳定的 runtime closure，不复制全部 `input_paths` 或 manifest/glob sibling。这些 exact 路径可以是 `planned` 并在 Preflight/Compile 时尚不存在，但 Final Gate 必须在当前 candidate 中看到它们；仅 materialize 不改变 Authority identity，未绑定且缺失的 repository-file argv token 则在 Compile 失败。Web/process 代理不能代替单独要求的 Native/desktop 目标。当前机器 `target_runtime` 只有 Harness 直接启动该 Source-backed process product root 才能成立；browser/native/desktop/device 要求保留阻断性 External Confirmation。每个 `critical_user_path` Outcome 和 Stage gate 都必须通过 admitted root proof 或对应 External Confirmation 处理每个 required target。
...
因此机制开发和发布结论只使用四级证据措辞：已设计、已实现、已建立声明中的已知反例保护、在明确 TCB 内高水准实现。当前 Level 3 黑盒集合包含 21 个 case：18 个 wrong candidate 均被拒绝，admitted static/process controls 为 `machine_accepted`，不支持且阻断 target 的 control 为 `blocked_external`；R9–R11b 复用既有 sentinel registry，Compile-time case 同时保留 owner diagnostic 和 legal-neighbor Final-Gate 拒绝。本轮 observer 修订在独立能力审计确认无开放 critical false acceptance、且一组真实 Long-Task process workload 配对证明总 ROI 为正之前仍最多为 Level 3。prose 审计、测试数量、fresh-Agent 实验和脱敏 fixture 都不能单独提升等级或证明真实事故代表性。

真实 process ROI 的 owner 是 `examples/delivery-benchmark/real-process-workload/**`、`tools/long_task_real_process_roi_{policy,runner,scoring}.mjs` 与 `tools/verify_long_task_real_process_roi.mjs`。它冻结 8 个 Fact、normal/degraded modes、两个 Counterfactual、独立 semantic gold 和 A/B/C 比较角色。真实采集仍等待 clean committed final C；authoring token 必须来自 invocation-bound host/provider usage event，缺失时记为 `required-unverified`，不接受 surrogate tokenizer，因此当前不能得出正 ROI 或 Level 4 结论。
