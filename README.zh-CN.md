...
实际应用的 register/move 共用当前字节与文件 identity CAS、同目录同步临时文件、以不可变摘要链 generation 发布的单一逻辑 no-replace v2 journal、完整 candidate-tree overlay 验证、确定性提交顺序、live after-state 复核，以及 `context transaction status|rollback|complete`。恢复会记录并比较完整 before/已发布端点快照（内容、mode 和平台可用的文件 identity），只接受 journal 精确拥有的部分临时文件，并拒绝外部冲突、无 owner 链接或后来激活的 Long-Task。Long-Task 首次 Compile、activation、原位 Revision 与 mutation recovery 复用既有 Active Authority lock 串行化，而且每条 Long-Task 写路径都拒绝未完成 journal。若崩溃恢复与已激活绑定冲突，恢复不会写入：先通过合法 Long-Task lifecycle 结束该绑定，再完成或回滚事务，最后重新 Compile/rebind，并执行任何必要的 Authority Revision。未完成的旧 v1 journal 会 fail closed，只能按匹配版本人工恢复，不能猜测转换。这里承诺的是协作进程边界内的确定性崩溃恢复，不是文件系统级跨文件物理原子性或 hostile-writer 互斥；不存在 `--force`、手工删锁或隐式全 Manifest normalize 路径。
