...
材料性的 DRA 修订循环只从绑定 raw-byte digest 的 Base 与完整有序 Delta 语义重放；`origin`、限定的 `decision_authority`、资源 `evidence_refs` 和 accepted/rejected/unresolved 裁决保持独立，因此资源被选中不能授权其自身建议的意义。只有真实跨中断需求才可显式用 `ty-context design-resource recovery create` 创建一个 ignored、任务局部、非权威 checkpoint；`inspect`/`preview` 重新派生当前状态，`apply` 通过写前/写后 raw-byte CAS 与新鲜的 Requirements→Resource、Resource→Requirements、Unexpected Blast Radius 审计写回，`remove` 只删除 digest 匹配且由 helper 所有的 checkpoint。简单 preview 不创建 checkpoint、持久字节、暂停、Provider run、正式 handoff、Proposal 写回或 helper transaction。checkpoint 与 reconciliation 只是上游恢复/诊断数据，绝不是 Design Authority、Long-Task Source/Evidence 或完成证明。
