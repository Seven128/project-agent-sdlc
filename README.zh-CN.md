...
更强的路线 B 观察边界不允许普通任务 verifier 仅凭自报 actual digest、comparison、`passed` 或 verdict 关闭 material/selected-design 结果。package-owned `json-pointer-exact-v1` 故意保持很窄：在当前 `process` 产品 Check 中，它从声明为产品实际消费的 UTF-8 JSON carrier 重新读取固定 RFC 6901 `/observations/<Fact-or-obligation identity>` locator，规范化值并重算 digest，再由 Harness 重算 `exact_value + exact` comparison identity。Source/Contract/expected 材料、comparison report、只被 verifier 读取的输出和历史 session 都不能充当 actual；构建生成的 bundle/config 只有同时属于产品 input/Binding carrier 时才可用。protected 值、tolerance/mask、custom locator、layout/accessibility/motion/pixel 以及 browser/native/live session 必须使用另一个由 Harness 直接调用且冻结的 adapter，或进入 External Confirmation。既有 `named_external_tcb` 集成仍可在明确收窄的“可信 verifier”定理下使用，但不会自动获得更强的路线 B 能力声明。本次有界 adapter 可由 v3 表达，不意味着发布 v4 或建设通用 UI observer。
...
因此机制开发和发布结论只使用四级证据措辞：已设计、已实现、已建立声明中的已知反例保护、在明确 TCB 内高水准实现。最高级还要求独立 ground truth 的正负控制、无开放 critical false acceptance、真实黑盒 Final Gate、要求的脱敏真实回放和正 ROI。prose 审计、测试数量和 fresh-Agent 实验都不能单独提升等级；fresh-Agent 配对只证明遵循率与成本。
