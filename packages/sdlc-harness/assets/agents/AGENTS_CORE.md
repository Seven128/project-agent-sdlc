...
- `SPRINTING` 阶段每完成一个 task，都必须形成单独的 task-level commit，并 push 到当前 upstream branch；push 成功前不进入下一个 task。
...
11. 在 `SPRINTING` 阶段，task 完成闭环必须包含 task-level `git commit` 和 `git push`；如果没有 remote/upstream、权限或凭证导致无法 push，不要开始下一个 task，先报告 blocker。
