...
- Harness authoring skills（如果存在）：`.agent/skills/authoring/`，只在维护 Harness/workflow/npm package 源码或本仓库自举规则时读取，不作为用户项目默认分发内容
...
- 后续更新 `.agent/skills/*/SKILL.md`、`.agent/skills/authoring/*/SKILL.md` 或 `.agent/pjsdlc_managed/templates/*.md` 时，遵循“中文解释 + 英文精确标识符”。Harness 根目录由 `package.json#sdlcHarness.harnessFolderName` 或 `sdlc-harness.config.json#harnessFolderName` 决定；未配置的项目默认使用 `.agent`。
