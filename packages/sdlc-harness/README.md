...
`plan.draft.yaml.tasks[]` means unadopted development drafts only. When `/dev` promotes a draft during `SPRINTING`, it creates the formal `TASK-*` in `plan.yaml` and removes the source draft in the same state update. The formal task is then recovered only from `plan.yaml`; completed history lives in implementation docs, git, PR and CI records. `/devloop` treats the development queue as exhausted only when both `plan.yaml.tasks[]` and `plan.draft.yaml.tasks[]` have no executable task.
