...
export async function context(args: string[]): Promise<void> {
  ...
  await contextInspect(rest);
}

async function contextInspect(args: string[]): Promise<void> {
  ...
  try {
    ...
    throw new CliCommandError(
      ...
      `context inspect failed internally: ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
  }
...
The 0.10 read-only release does not include create, register or move.`;
}
