...
每一种 non-interference 方法都必须使用带 digest 的 frozen executable Oracle，并声明精确的 `symbolic_noninterference.<side>.<method>` capability。静态证明不能由轴列表自证：其 Oracle 必须针对 Source/production 两侧分别产出覆盖精确冻结输入的 dependency DAG，preflight 对共享子图做 memo 并重算循环、悬空/不可达节点、输入闭包和每个 Rule root 的传递轴集合。受限 IR/有限穷举则证明 `Rule region = side predicate = axis-erased predicate`。两侧 proof digest 进入 certificate identity，并在存在省略轴时进入既有 Contract expectation/current Final-Gate result；任意 Oracle 的抽取正确性仍是明确的 TCB 边界。
