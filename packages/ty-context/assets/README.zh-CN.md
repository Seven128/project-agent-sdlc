...
`route` 是实验性、无状态的字面量候选诊断，会扫描全部符合条件的 `project_context/**`，并明确标记未登记文件。其版本化 JSON、固定候选/内容/输出预算、稳定顺序、选择原因与字节成本不建立 Authority、不参与默认 footprint，也不能替代 Workflow 仍然必需的 bounded search；预算超限会显式返回不完整结果。这些 Router 预算是在 Shared Catalog 完成仓库级 Context 发现与元数据收集后才生效；0.10 刻意保持这一步完整遍历。未来若增加总发现预算，必须由 Catalog 统一拥有、向所有消费方传播类型化的不完整状态，并让 Router 返回 `complete: false`/退出码 `4`；禁止另建 Router 专用 walker，也禁止静默截断 Validate/Doctor。`context inspect` 只读报告单个文件的登记状态、Role、读取元数据、默认选择原因、显式反向引用、稳定 key 冲突和可选路由解释。
...
实际应用的 register/move 共用当前字节与文件 identity CAS、同目录同步临时文件、完整 candidate-tree overlay 验证、确定性提交顺序、live after-state 复核，以及 `context transaction status|rollback|complete`。当前写入使用 `context-mutation-journal-v3`：不可变摘要链 generation 会把每个 NFC 逻辑 key 绑定到精确物理路径；`context-mutation-journal-v2` 仍可恢复读取，并把逻辑路径与物理路径视为相同。pre-v2 journal 必须使用匹配版本或显式人工恢复，绝不猜测或重写。恢复会记录并比较完整 before/已发布端点快照（内容、mode 和平台可用的文件 identity），只接受 journal 精确拥有的部分临时文件，并拒绝外部冲突、无 owner 链接或后来激活的 Long-Task。Long-Task 首次 Compile、activation、原位 Revision 与 mutation recovery 复用既有 Active Authority lock 串行化，而且每条 Long-Task 写路径都拒绝未完成 journal。若崩溃恢复与已激活绑定冲突，恢复不会写入：先通过合法 Long-Task lifecycle 结束该绑定，再完成或回滚事务，最后重新 Compile/rebind，并执行任何必要的 Authority Revision。这里承诺的是协作进程边界内的确定性崩溃恢复，不是文件系统级跨文件物理原子性或 hostile-writer 互斥；不存在 `--force`、手工删锁或隐式全 Manifest normalize 路径。
...
未决 substrate observation 是 target-wide 事实，而不是抽样信息：每个 `decision_required` 或 `unavailable` observation 都必须由每个 material family × condition cell 的精确 blocker 引用，blocker 与 marked-Source projection 的 observation-ref 集必须完全一致，零 family 时也不能真空通过。带候选的 cell 必须有 observed component-owner roots。observation reason、cost、risk 与 blocker description 共用一个时间分类器，识别紧凑或带空格、单复数的 `millisecond|msec|ms|second|sec|s`：出现 motion 上下文一律阻断；明确的 build/compile/test/startup/latency/timeout/network/benchmark/runtime/render cost 上下文合法；没有上下文的时间字面量 fail closed。精确动效时间或语义不明的时间值仍归 canonical resource。

Long-Task 消费时，系统会逐条证明每个 `file`/`path_glob` target 与 carrier、每个 `verified` carrier、以及每个 planned 非空 carrier 完全位于 observed component/route roots 内；部分匹配或无法证明都失败。独立 Contract 结构校验仍拒绝空 component refs。只有完整 activation 调用可以暂缓这一条，并且仅当 surface 的每个 target 都是 modern blocker-only、route 与精确 blocker 闭包有效时通过；legacy target 不能借用该例外。纯 legacy surface 保留历史上的非空 component-Binding 兼容，但不获得 feasibility 声明。surface 一旦出现 modern feasibility target，每个 component Binding 都必须由 modern candidate-bearing cell 的实际唯一匹配消费；legacy 消费空集，因此不能在 mixed surface 中洗入额外 Binding，而多个 modern target 可以共享各自明确匹配的同一 Binding。开放的 target-blocking External Confirmation 只能到达 `blocked_external`，不能 `machine_accepted`，也不会增加或改变唯一 Final Gate。
...
它的证明命题有明确前提：Source 在声明观察粒度下完整且准确、Source→Contract 投影保持语义、所有实际适用单元被展开。在此前提下，`AcceptedDeliveryTerminal` 只有两种：不需要阻断性外部履约时的 fresh `machine_accepted`，以及全部阻断性外部义务已有 fresh、有效、逐义务精确记录时的 `delivery_accepted`。机器义务仍必须具有冻结 Expected 权威、package-admitted 当前 Actual、Harness 计算的 comparison/verdict、可归因静态生产载体或直接 process observation、因果 Counterfactual 与当前 Final Gate 快照证明。每份阻断性 External Record v2 都携带逐义务身份/证据、当前 challenge、whole-candidate 身份、不可变 artifact snapshot，以及由受保护 Source 公钥校验的 detached Ed25519 签名；这证明签名者持有绑定私钥，但不会在 Source 绑定之外独立证明真实世界身份。Record v1 与旧 `machine_accepted_external_pending` 仅可审计，不能关闭 Authority。Harness 无法发现未声明要求，也不能声称任意物理/外部观察绝对可靠。

Compile 会在 Source Item 之下继续守恒语义：从既有 Semantic Fact Manifest 派生一个内部视图，覆盖全部 Source Item，以及可严格按 UTF-8 读取的 attachment、canonical specification、repository-preservation input、external constraint、delegated instruction 和文本型 design resource；它不按扩展名猜测，无法读取或类型含糊的文本会阻断。扫描器覆盖标题、段落、嵌套列表、表格、引用、Given/When/Then、代码/配置块和文本 HTML，并证明全部非空材料行恰好等于互不重叠的 Fragment 行与允许的纯分隔符行之并。每个 Fragment 恰有一个显式 `fact_bearing | supporting_basis | superseded | decision_required | scope_excluded` disposition；反向 delivery-Fact provenance、Anchor、semantic-cell identity 与 Claim modal polarity 防止 supporting、supersession 或 exclusion 隐藏独立要求。`source_integrity` Fact 不能代替 `delivery_semantic` 含义；这仍复用既有 Source inventory 与 Semantic Fact Manifest，不建立第二份总账。

Proof Adequacy 同样是结构约束：封闭的 method 与标准 property profile 会同 observation-scope、quantifier、Claim-kind 和 Control-field floor 取并集，文本关键词只能诊断，不能改变权威。presence 只能证明存在；transition/sequence、产品结果、边界 provenance、持久化 readback、身份/数据状态、population、fault/recovery、concurrency/idempotency、performance/capacity、选定设计与目标运行时分别保留更强 typed capability。新增但未分类的标准 property 会失败；无标准 profile 的 `custom.*` method/property 不能 Machine close。Preflight 先按精确 Claim/Fact × method × applicability obligation 生成 machine/external 候选，再要求恰好一条路线；machine/external 重叠、同类多路或无路线均为 `unreachable`，宽泛 `impact_claims` 不得删除 machine Assertion。
...
- `external prepare` 输出当前候选 Packet、current challenge、Source-bound actor/key identity、signable canonical payload digest、逐义务 Expected/method/capability 与证据要求，并明确不产生接受。`external submit` 接受严格 `long-task-external-confirmation-record-v2`，依次校验 Active Authority/freshness、schema、whole candidate、challenge、actor、Ed25519 签名、精确义务/result-kind 集、不可变 artifact snapshot 与客观 comparator 后原子保存。`external status` 重算 identity/signature/challenge/artifact integrity；`external rotate` 轮换 challenge；`external revoke` 删除命名记录并轮换 challenge。这些写操作与 finalization 共用 Active Authority lock。failed/unable、declared-only actor 与 Record v1 均不能接受。
- 程序化 v2 记录生产者可从 package 根入口导入 `parseExternalConfirmationRecordV2`、`externalConfirmationRecordV2Hash` 与 `externalConfirmationV2SignablePayload`，在 Harness 外使用 actor 自持私钥签署 canonical payload，再提交完整记录。Harness 只提供验证，不提供私钥读取或 v2 代签命令/helper。导出的 v1 parser/hash/`signExternalConfirmationRecordV1` 只用于 legacy 审计兼容；其本地哈希不是认证，也不能完成阻断义务。
- `final-gate` 要求 clean candidate commit，拒绝 stale Authority 输入，重编译 Source Authority，捕获完整受保护输入与 provisional Finalization Identity，并在一个 Git-tree snapshot 上只运行一次全部 semantic Check。随后进入现有 lock 下的短 Finalization CAS，不重跑 machine Check，而是重查 Authority、candidate/workspace fingerprint、compiled/verifier freshness、全部 external record/challenge/signature/artifact/evaluation 与精确 Finalization Identity。Final Receipt v3 绑定 `finalization_identity_sha256`，只产生 `machine_accepted`、`delivery_accepted`、`blocked_external` 或 `needs_work`。
- `stop-check` 与 `close` 运行同一个 Live Final Gate，并在同一事务中发布 Receipt；只有 fresh accepted identity 才清理 Active Authority。任一覆盖竞态都会产生非接受结果、保留 Authority，且不能留下 fresh accepted Receipt。缺少 `finalization_identity_sha256` 的历史 v3 Receipt、旧 `machine_accepted_external_pending`、pending、failed、stale 或 invalid 均只可审计并 fail closed。Final/Stop/close 输出 `acceptance_scope: declared_delivery_authority` 与 `native_goal_effect: none`，close 另输出 `closed_scope: complete_long_task_authority`。
...
0.11 增加可选稀疏 Design Authority bundle，但不自动迁移既有项目。只有未标记根 `DESIGN.md` 的项目仍是合法单文件 closure；采用 Bundle 是一次显式 DSA/Authority Revision，必须同时插入规范 `bundle-v1` marker 并创建 `design_system/authority.manifest.json`，`tokens.json` 从 DESIGN 生成而不是第二个可编辑 owner。旧 DRA handoff 只有在项目仍是真实单文件 closure 时才可省略完整身份；采用后保留任意一半却删除或损坏另一半都会 fail closed，原始 `DESIGN.md`-only binding 必须按完整当前 closure 重新生成或 rebind。回到单文件 Authority 必须通过显式 DSA adoption/Authority Revision 同时移除两半；package 能验证当前 marker/manifest 配对并使旧 active binding 失效，但一个两者都不存在的新 checkout 没有认证历史 registry 可供推断曾经的 bundle。Upgrade 不会发明子 owner、Token 值、adoption 或 Authority revision。
