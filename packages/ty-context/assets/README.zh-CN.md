...
真实 process ROI 的实现 owner 是 `examples/delivery-benchmark/real-process-workload/**`、`tools/long_task_real_process_roi_{policy,runner,scoring}.mjs`、`tools/long_task_formal_total_cost_evidence.mjs` 与 `tools/verify_long_task_real_process_roi.mjs`。它冻结 8 个 Fact、normal/degraded modes、两个 Counterfactual、独立 semantic gold、A/B/C 比较角色及每个 fixture 的已提交候选 identity。V3 总是采集五个冻结的 A/B/C repeat，前三组 expansion 结果只作诊断。每个 fixture 生命周期前后的 HEAD/tree/status 原始命令记录绑定该 identity，并要求候选保持 clean 且不变。V3 严格分 owner：workload/collection 只产生 immutable raw/observed facts；有界 packet 只携带 raw external evidence；aggregate/attestation 的 formal status 只能是 `not_evaluated`；只有现有 verifier 验证 evidence、重算 normalized exclusive events 并输出 `total_roi_supported`、`total_roi_positive` 与 formal status。Packet 自报的 `verified`、normalized value、货币化事故总额或 event ID 会被拒绝。Verifier 绑定 exact run-set/run、A/B/C commit/tree/package identity、collector closure、采集前哈希的 `accounting-policy.json`、runner 物化的 collector/price/redaction precollection identity、有界 regular no-follow source manifest 与 retention/redaction。本轮冻结口径为：1 NCU=CNY 1；active/wait 的 CNY 200/50 每小时只作本轮代理；一个周期 10 次交付；5 组 B/C 交付中位数投影 10 次；maintenance/recovery 的 5 组中位数各计 1 次；introduction/adoption/migration 各计 1 次；一个固定事故的 5 组 benefit 中位数计 1 次。Provider/compute/storage 由 prelocked 机器可读账单或官方价格文档重算，事故 benefit 由 raw time/usage 重算。显著稳定余量要求 benefit 至少为正增量成本的 1.25 倍；五个 benefit pair 各减同一个代表性正周期成本；至少 4/5 paired net 为正且 sample CV 不超过 20%；成本下降另报。缺失证据或 precollection lock 即 unsupported。dry-run/collection 用 `--formal-evidence-plan <plan>` 传入计划，只有 report verification 可通过 `--formal-evidence <packet>` 传入 packet，collection 不评价 packet。历史 v1/v2 不能证明 v3；机器测量、独立审计与项目所有者 promotion 批准保持分离。
