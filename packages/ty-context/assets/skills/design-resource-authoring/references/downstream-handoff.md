...
UI symbolic V2 is explicit opt-in; V1 remains the default. Do not infer V2 from file size, provider output or a capacity diagnostic. Only an explicit per-target selection may emit `design-resource-observable-rule-manifest-v2` plus `design-resource-handoff-v2` / `representation: symbolic_rules_v2`. Keep one constant canonical-resource located expected value per Rule and mutually exclusive exhaustive effective regions. Applicability may use exact physical remainder partitions or the package profile catalog with a frozen Inspector custom-property Census closure and explicit unique instance exceptions; every logical subject-property point remains dispositioned even when no N/A row is stored. Compute Rule and proof-obligation identities separately, then attach set-valued Rule/omitted-axis certificates whose canonical DAG metrics and both Source-side and production-side non-interference proofs are recomputed by package preflight. Only frozen closed-world static dependency closure, restricted-IR exact equivalence or finite complete-domain exhaustive equivalence may prove an omitted axis; dynamic loading, reflection, unfrozen implicit inputs, external devices, representative points and sampling block. V2 authoring never lowers granularity, expands then deduplicates or claims production acceptance. The V1 bundle path remains V1-only; explicitly authored V2 is preflighted directly until a later separately admitted publisher exists.

For every non-interference method, bind a digest-identified frozen executable Oracle that declares the exact `symbolic_noninterference.<side>.<method>` capability. Source authoring must place one canonical, non-executable `design-resource-symbolic-source-ir-v1` resource for the admitted target/certificate/Rule scope in the complete Inspector inputs. That IR carries Source predicates, not submitted proof conclusions. Package preflight reparses current IR bytes and derives and memoizes the static dependency DAG, canonical predicate/axis-erased equivalence or every finite-domain evaluation; submitted graph/predicate/evaluation/pass fields and the proof artifact are only recomputable caches. Do not derive Source graph axes from Rule refs. JavaScript, CSS/implicit DOM, executable templates, dynamic/reflected/computed access, unfrozen extensions and external runtime/device Source remain blocking unless a later package-owned complete extractor is admitted. Production remains inside the package-parsed static HTML plus inert JSON subset. Both sides bind the Oracle implementation closure/version/capability, environment, every input path and digest, side snapshot, Rule/certificate scope, omitted axes, derived result, artifact and witness. Source and production proof digests are downstream certificate bindings; extraction outside admitted representations remains an explicit TCB boundary.
