...
UI symbolic V2 is explicit opt-in; V1 remains the default. Do not infer V2 from file size, provider output or a capacity diagnostic. Only an explicit per-target selection may emit `design-resource-observable-rule-manifest-v2` plus `design-resource-handoff-v2` / `representation: symbolic_rules_v2`. Keep one constant canonical-resource located expected value per Rule, mutually exclusive exhaustive effective regions, the full package property catalog, frozen Inspector capability/Census support and basis-backed remainder dispositions. Compute Rule and proof-obligation identities separately, then attach one or more set-valued certificates whose exact omitted-axis/dependency-edge coverage and canonical DAG metrics are recomputed by package preflight. V2 authoring never samples, lowers granularity, expands then deduplicates or claims production acceptance. The V1 bundle path remains V1-only; explicitly authored V2 is preflighted directly until a later separately admitted publisher exists.
