...
For every non-interference method, bind a digest-identified frozen executable Oracle that declares the exact `symbolic_noninterference.<side>.<method>` capability. A static proof additionally carries a side-specific dependency DAG over the exact frozen input set; package preflight memoizes and recomputes graph/input closure and every Rule root's transitive axes. IR/exhaustive cases carry the side predicate needed to prove `Rule region = side predicate = axis-erased predicate`. Source and production proof digests are downstream certificate bindings; Oracle extraction soundness remains the named TCB boundary.
