...
- A Web/App implementation resource must be feasible on the real project substrate. DRA may enumerate Source-backed feasible realizations and blockers, but it cannot select a production owner or required realization unless current technical authority already makes that choice.
...
5. For a Web/App implementation handoff, inspect current technical Source for platform, framework/runtime, UI system, token/theming adapter, component-owner roots and route-owner roots. Cover every material component-family × target × condition profile with at least one Source-backed feasible realization or an explicit blocker. Keep canonical design values in design resources; never put them into the feasibility document.
