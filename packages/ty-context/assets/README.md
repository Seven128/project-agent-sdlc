...
Compile derives an internal `CompiledObservationAuthority` projection for every machine Claim or Fact × required-method obligation; it is not a new Contract Authority, state or registry. The first admitted slice has only two machine paths. `package_static_json_exact` reads plain exact implementation/configuration content from a UTF-8 JSON production carrier that already exists in the pre-run snapshot, retains the same no-follow file identity/digest after the runner, matches the Binding and is not Source/Context/Contract/expected material or evidence/report/status/verifier output; Harness selects the fixed RFC 6901 `/observations/<stable Fact-or-obligation identity>` locator and applies package duplicate-key/UTF-8/size/depth/pointer limits. It proves static content, not runtime consumption. `package_process_json_exact` applies only to `runtime_family: process`, `role: product`, a direct root `project_binary` with no wrapper and read-only/test-sandbox effect. Harness owns the process handle and an exclusive OS-temporary `ty-context-product-observation-v1` channel, supplies a nonce for replay freshness, extracts multiple current Actuals, computes every exact result, derives host runtime/interaction/state evidence and cleans the channel. The public project payload remains v3; no v4 or general UI/native observer is introduced.

Project-submitted actual/value digest, comparison, `passed`, verdict, `target_runtime`, `interaction_trace` and `state_delta` are compatibility diagnostics only and must agree if present. Custom/`named_external_tcb` Oracles, wrappers, browser/native/device sessions, layout/pixel/accessibility/motion, protected observation, tolerance/mask and custom locators cannot machine-close an obligation and require blocking External Confirmation. Every machine Counterfactual needs admitted baseline and mutated observations, an attributable production carrier, exact affected/preserved/allowed-fan-out accounting, equal obligation universes and host-derived process liveness where applicable; no-observation never skips validation. Existing Contracts are not silently rewritten, and observer/TCB changes invalidate prior Active Authority, Progress and Receipts for acceptance.
...
The Contract declares one bounded target profile, its non-empty required product target refs and each target's runtime family, root entrypoint and explicit capabilities. Required product targets name their family capability plus `cold-start` and `production-root`. A Web/process proxy cannot satisfy an independently required Native/desktop target. Current machine target-runtime proof exists only when Harness directly spawns the declared process product root; browser/native/desktop/device requirements remain target-blocking External Confirmations. Every `critical_user_path` Outcome and Stage gate accounts for every required target through admitted root proof or that External Confirmation.

When a declared result can pass on a proxy surface while failing in its target runtime, the earliest owning Outcome carries either an admitted direct-process root Check or a blocking External Confirmation. A project payload, tracked report, screenshot, binary, log, historical run, new session id or proxy cannot be runtime authority. Checks still declare keyed Given/When scenarios and exact applicability; every Claim-bearing Assertion remains independently attributable without sampling. Project capability records are diagnostic compatibility data, while package-derived observations satisfy the all-of capabilities. Static structure cannot prove behavior. Every behavioral machine Assertion uses a same-Check Counterfactual whose admitted affected Facts change, preserved Facts/liveness do not, other changes are explicit fan-out and baseline/mutated obligation universes are equal. A Binding or path is not reachability proof: static mutation proves only that structure, while runtime reachability requires Harness mutation → direct product-root execution → package-observed Actual change. Pure evidence/status/report/Receipt/verifier output cannot be a production carrier. The remaining runner identity, minimal invalidation-envelope, targeted-feedback and current Final-Gate rules are unchanged; this adds no generic reachability scanner, implementation gate, scheduler or state.
...
Mechanism and release wording therefore has four evidence levels: designed, implemented, protected against the declared known counterexamples, and high-quality realization within an explicit TCB. This observer revision remains at Level 3 until an independent capability audit confirms no open critical false-acceptance path and a paired real Long-Task process workload demonstrates positive total ROI. Only then may Level 4 be restored, together with independently grounded positive/negative controls and a real black-box Final Gate. Prose review, test counts, fresh-Agent pairs and sanitized fixtures cannot promote the level or prove real-incident representativeness; Agent pairs measure adoption and cost only.
