...
The real-process ROI owner is `examples/delivery-benchmark/real-process-workload/**` plus `tools/long_task_real_process_roi_{policy,runner,scoring}.mjs`, `tools/long_task_formal_total_cost_evidence.mjs` and `tools/verify_long_task_real_process_roi.mjs`. It freezes eight Facts, normal/degraded modes, two Counterfactuals, independent semantic gold, A/B/C comparison roles and each fixture's committed candidate identity. V3 always collects all five frozen A/B/C repeat orders; the initial-three expansion result is diagnostic only. Exact raw HEAD/tree/status command records before and after each fixture lifecycle bind that identity and require the candidate to remain clean and unchanged. V3 separates ownership: workload and collection produce immutable raw/observed facts; a bounded packet carries raw external evidence; aggregate and attestation formal status is `not_evaluated`; only the existing verifier validates evidence, recomputes normalized exclusive events and emits `total_roi_supported`, `total_roi_positive` and formal status. Packet-authored `verified`, normalized values, monetary incident totals and event IDs are rejected. The verifier binds exact run-set/run and A/B/C commit/tree/package identities, collector closure, the hashed pre-collection `accounting-policy.json`, a runner-materialized precollection identity for collector/price/redaction inputs, bounded regular no-follow source manifests and retention/redaction. The frozen policy is specific to this delivery: 1 NCU=CNY 1; active/wait time uses CNY 200/CNY 50 per hour as accounting proxies only; one cycle is ten deliveries; five B/C delivery pairs project by their median to ten; maintenance and recovery five-pair medians each count once; introduction/adoption/migration each count once; and one controlled-incident five-pair median benefit counts once. Provider/compute/storage conversion is derived from prelocked machine-readable invoice or official-price documents, while incident benefit derives from raw time/usage. Significant stable margin requires benefit at least 1.25 times positive incremental cost; each benefit pair subtracts the same representative positive cycle cost; at least 4/5 paired nets must be positive and sample CV at most 20%. Cost reductions are reported separately. Missing evidence or precollection lock remains unsupported. Pass the plan to dry-run/collection with `--formal-evidence-plan <plan>` and the packet only to report verification with `--formal-evidence <packet>`; collection never evaluates it. Historical v1/v2 reports cannot prove v3. Machine measurement, independent audit and explicit project-owner promotion remain separate.

Formal collection additionally prelocks one fixed scenario catalog with exact task/gold bytes for every ten-cost scenario and the one controlled incident. Every event binds a unique raw output: both cost variants must match their common gold, while incident B must differ and C must match it. This prevents time/usage-only purpose claims without adding a generic scenario registry.
...
`npm run preview:pack` produces a local preview named `project-tiny-context-harness-0.8.14.tgz` under the preview output directory.
