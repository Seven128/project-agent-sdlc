...
Workflow changes pass two hard gates in order. First prove `Coverage_new ⊇ Coverage_old`, `FalseNegative_new ⊆ FalseNegative_old` and preserve non-bypassable Authority, fail-closed behavior and complete-current-final-snapshot proof; if that cannot be proved, preserve the current formal acceptance path. Cost savings never compensate for weaker drift detection. Only then compare total incremental benefit with all incremental authoring, runtime, state, recovery, maintenance, test, process, introduction and migration cost. Positive ROI means benefit exceeds all those costs and the proposal may enter the consideration set; it never means automatic adoption.
