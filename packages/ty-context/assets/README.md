...
For the stronger Route-B observation boundary, an ordinary task-authored verifier cannot establish a material or selected-design result merely by submitting its own actual digest, comparison, `passed` flag or verdict. The package-owned `json-pointer-exact-v1` adapter is deliberately narrow: on a current `process` product Check it re-reads a declared product-consumed UTF-8 JSON carrier at the fixed RFC 6901 `/observations/<Fact-or-obligation identity>` locator, canonicalizes the value, recomputes its digest and lets Harness recompute `exact_value + exact` comparison identity. Source/Contract/expected material, comparison reports, verifier-only output and historical sessions are ineligible actual carriers; a generated bundle or configuration remains eligible only when it is also a declared product input/Binding carrier. Protected values, tolerance/mask, custom locators, layout/accessibility/motion/pixel observations and browser/native/live sessions require a separately frozen directly invoked adapter or External Confirmation. Existing `named_external_tcb` integrations remain usable under their explicitly narrower trusted-verifier theorem, but do not by themselves acquire the stronger Route-B capability claim. This bounded adapter fits v3; no v4 or general UI-observer protocol is implied.
...
Mechanism and release wording therefore has four evidence levels: designed, implemented, protected against the declared known counterexamples, and high-quality realization within an explicit TCB. The highest level additionally requires independently grounded positive and negative controls, no open critical false-acceptance path, a real black-box Final Gate, the required sanitized-real replay and positive ROI evidence. Prose review, test counts and fresh-Agent experiments cannot promote the level by themselves; fresh-Agent pairs measure adoption and cost only.
