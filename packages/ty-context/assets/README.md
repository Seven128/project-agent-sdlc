...
Material DRA revision loops replay from a raw-digest-bound Base plus complete ordered Delta semantics; `origin`, bounded `decision_authority`, resource `evidence_refs` and accepted/rejected/unresolved status stay independent, so selecting a resource cannot authorize its own suggested meaning. A real cross-interruption need may explicitly create one ignored, task-local, non-authoritative checkpoint with `ty-context design-resource recovery create`; `inspect` and `preview` rederive current state, `apply` uses pre/post raw-byte CAS plus a fresh Requirements→Resource, Resource→Requirements and unexpected-blast-radius audit, and `remove` deletes only the digest-matched helper-owned checkpoint. A simple preview creates no checkpoint, persisted bytes, pause, Provider run, formal handoff, Proposal write or helper transaction. The checkpoint and reconciliation are upstream recovery/diagnostic data, never Design Authority, Long-Task Source/Evidence or completion proof.
