...
Compile derives an internal `CompiledObservationAuthority` for every machine Claim or Fact × required-method obligation; it is not a new Contract Authority, state or registry. `package_static_json_exact` reads plain exact implementation/configuration content only from a UTF-8 JSON production carrier that exists in the pre-run snapshot, retains its no-follow identity/digest after the runner, matches the Binding and is not Source/Context/Contract/expected/evidence/report/status/verifier output; Harness selects the fixed RFC 6901 `/observations/<stable Fact-or-obligation identity>` locator and applies package duplicate-key/UTF-8/size/depth/pointer limits. It proves no runtime consumption. `package_process_json_exact` supports only `runtime_family: process`, `role: product`, a direct root `project_binary` with no wrapper and read-only/test-sandbox effect. Harness owns the process and exclusive OS-temporary `ty-context-product-observation-v1` channel, uses a nonce only for replay freshness, extracts multiple current Actuals, computes exact results, derives host runtime/interaction/state evidence and cleans the channel. Project results remain v3; no v4 or general UI/native observer is introduced.

Project-submitted actual/value digest, comparison, `passed`, verdict, `target_runtime`, `interaction_trace` and `state_delta` are compatibility diagnostics only and must agree if present. Custom/`named_external_tcb` Oracle, wrapper, browser/native/device, layout/pixel/accessibility/motion, protected, tolerance/mask and custom-locator observation require blocking External Confirmation. Machine Counterfactuals require admitted baseline/mutated observations, an attributable production carrier, exact affected/preserved/allowed-fan-out accounting, equal obligation universes and host-derived process liveness when applicable; absence of admitted observation never skips validation. Migration never silently changes the observer choice, and observer/TCB changes invalidate prior Active Authority, Progress and Receipt acceptance.
...
When a declared result can pass on a proxy surface while failing in its target runtime, the earliest owning Outcome carries either an admitted direct-process root Check or blocking External Confirmation. A project payload, report, screenshot, binary, log, historical run, new session id or proxy cannot be runtime authority. Applicability and all Claim/proof cells remain independently attributable; project capability records are diagnostic, while package-derived observations satisfy the all-of capabilities. Every behavioral machine Assertion has a same-Check Counterfactual whose admitted affected Facts change, preserved Facts/liveness do not, other changes are explicit fan-out and obligation universes are equal. A Binding/path is not reachability proof: static mutation proves only that structure; runtime reachability requires Harness mutation → direct product-root execution → package-observed Actual change. Evidence/status/report/Receipt/verifier outputs cannot be production carriers. Existing minimal invalidation-envelope, targeted-feedback and current Final-Gate rules remain; no generic reachability scanner, implementation Gate, scheduler or state is added.
...
Mechanism/release claims progress only through designed, implemented, known-counterexample protection, and high-quality realization within an explicit TCB. This observer revision remains Level 3 until an independent capability audit confirms no open critical false acceptance and a paired real Long-Task process workload establishes positive total ROI. Only then may Level 4 be restored with grounded positive/negative controls and a black-box Final Gate. Prose, test counts, fresh-Agent pairs and sanitized fixtures cannot promote it or prove real-incident representativeness.
