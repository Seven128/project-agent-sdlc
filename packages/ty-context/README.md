...
Enabling Long-Task additionally installs `long-task-workflow`, the completion Hook and, only when the resolved harness root is exactly `.codex`, one optional project-scoped Codex custom agent named `long_task_implementation`. The fixed, stateless profile uses `gpt-5.6-luna`, `model_reasoning_effort = "max"` and disabled child-agent tools. It is usable only for bounded rolling implementation/repair after the first-Authority-Lock terminal-turn checkpoint. It is not a Skill, runtime, model router, scheduler, Authority or proof carrier. A missing or invalid package asset is skipped; symlinked or no-marker same-path user content is preserved; an exact-marker regular package-owned profile can be refreshed or removed. Absence falls back to parent execution without changing acceptance; base design Skills and user entries remain. Tiny Context does not install Open Design or another design-generation runtime.
...
5. Select `$long-task-workflow` with the exact paths to all of those inputs. It authors the Source-bound Contract Draft. The first Compile/Authority Lock always ends the current turn before implementation and says `After handling the model change, send [continue].`; earlier model wording cannot skip this boundary and Harness cannot observe whether the host model changed. After the user resumes, the parent evaluates delegation suitability and proactively uses `long_task_implementation` for independent bounded work when the benefit exceeds coordination cost; the parent still owns authority, architecture, Context, integration, current-candidate checks and formal verification.
