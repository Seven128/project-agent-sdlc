...
Every non-interference method requires a digest-identified frozen executable Oracle with the exact `symbolic_noninterference.<side>.<method>` capability. Static non-interference is not accepted from an axis list: its Oracle emits a side-specific dependency DAG over the exact frozen inputs, and preflight memoizes and recomputes graph/input closure and each Rule's transitive axes. IR and finite-exhaustive proofs instead prove `Rule region = side predicate = axis-erased predicate`. Source and production proof digests enter certificate identity and the existing current Final-Gate certificate expectation/result; arbitrary extraction soundness remains the named Oracle TCB boundary.
