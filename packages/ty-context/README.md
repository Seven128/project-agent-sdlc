...
Material DRA revision loops replay from a raw-digest-bound Base plus complete ordered Delta semantics; `origin`, bounded `decision_authority`, actual digest-bound Source items, resource `evidence_refs` and accepted/rejected/unresolved status stay independent, so selecting a resource or supplying an arbitrary ref cannot authorize suggested meaning. Resource decisions bind every requirement key to exact Delta/origin/authority/Source, complete changed/unchanged/resource-decision/blast-radius universes fail closed, and every accepted decision has exactly one Proposal or exact-visual resource owner. A real cross-interruption need may explicitly `create` one ignored, task-local, non-authoritative checkpoint; `update` replaces it only through caller-supplied checkpoint digest CAS, while `inspect` and `preview` rederive current state. `apply` uses pre/post raw-byte CAS plus a fresh Requirements→Resource, Resource→Requirements and unexpected-blast-radius audit. `remove` fully deletes only after inventory proves the directory contains the digest-matched helper checkpoint; otherwise it returns `partial` and preserves unowned content. A simple preview creates no checkpoint, persisted bytes, pause, Provider run, formal handoff, Proposal write or helper transaction. The checkpoint and reconciliation are upstream recovery/diagnostic data, never Design Authority, Long-Task Source/Evidence or completion proof.
