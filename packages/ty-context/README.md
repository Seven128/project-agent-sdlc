...
Long-Task Anti-Degradation Assurance protects both current causal-chain truth and cross-version interception strength. Current-implementation Context must match the indexed code/runtime, which must still realize the two necessary meaning-capture and fail-closed final-proof responsibilities under the explicit theorem boundary. Weakening the protected purpose, key logic, either responsibility or that boundary requires an explicit project-owner design-purpose decision and replacement proof. Existing Context, indexes, tests, critical sentinels, routing and parity gates carry this assurance without another Authority, Gate or state; they cannot infer omitted requirements or guarantee immutability against deliberate fully authorized joint weakening.
