...
The stronger Route-B boundary does not accept an ordinary task verifier's self-reported actual digest, comparison or verdict as sufficient material/selected-design proof. `json-pointer-exact-v1` re-reads a current product-consumed UTF-8 JSON carrier for a `process` target at the fixed RFC 6901 `/observations/<Fact-or-obligation identity>` locator, canonicalizes and hashes the value, and leaves `exact_value + exact` comparison/result identity to Harness. Expected/report-only/historical carriers are rejected; generated product inputs remain eligible. Protected, tolerance/mask, custom, layout/accessibility/motion/pixel and browser/native/live observations need another directly invoked frozen adapter or External Confirmation. Legacy `named_external_tcb` integrations retain only their narrower trusted-verifier claim. The bounded adapter uses v3 and is not a general UI observer.
...
Mechanism/release claims progress only through designed, implemented, known-counterexample protection, and high-quality realization within an explicit TCB. The highest level also requires grounded positive/negative controls, no open critical false acceptance, a black-box Final Gate, the required sanitized-real replay and positive ROI. Prose, test counts and fresh-Agent pairs cannot promote it alone; Agent pairs measure adoption and cost only.
