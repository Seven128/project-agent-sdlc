...
Direct process Compile uses one declaration-stable closure of exact Source-backed root/argv paths and exact production carriers. It never copies all `input_paths` or manifest/glob siblings. Exact planned members may be absent during Preflight/Compile but must exist at Final Gate; materializing them does not by itself revise Authority, while an unbound missing repository-file argv token fails Compile. Compatible Cross-Check Facts reuse one Raw Execution/stdout envelope while keeping independent exact results.
...
Mechanism/release claims progress only through designed, implemented, known-counterexample protection, and high-quality realization within an explicit TCB. The current Level 3 black-box set has 21 cases: 18 wrong candidates are rejected, admitted static/process controls reach `machine_accepted`, and the unsupported target-blocking control reaches `blocked_external`; R9–R11b reuse the existing sentinel registry and legal-neighbor Final-Gate route. This observer revision remains Level 3 until an independent capability audit confirms no open critical false acceptance and a paired real Long-Task process workload establishes positive total ROI. The repository-owned eight-Fact A/B/C benchmark is prepared, but authentic collection waits for a clean committed final C and authoring tokens remain `required-unverified` without an invocation-bound host/provider usage event. No surrogate tokenizer, prose, test count, fresh-Agent pair or sanitized fixture can promote Level 4.
