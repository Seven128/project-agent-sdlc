...
The separate opt-in DRA visual diagnostic is never an admission run. Start with `node examples/delivery-benchmark/mechanism/runner/visual_diagnostic.mjs freeze-check`, then prepare a blinded schedule exactly as documented in `visual-diagnostic/README.md`. Bind all five routes to immutable execution identities, use all eight fixed inputs, run at least three repeats, keep the private mapping from evaluators, complete every rubric dimension and retain raw limitations. Do not calculate a winning Provider, threshold, majority rule, admission aggregate, automatic routing choice or publication block from those reviews.
