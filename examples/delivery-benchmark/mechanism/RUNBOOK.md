...
After both unchanged-freeze aggregates exist and the candidate is a clean `main` commit, run `attest` with the deterministic report and both aggregate reports. The resulting manifest binds their digests, bounded metrics, provenance qualification, trace-identity set and exact `main` commit/tree. It intentionally excludes prompts, model output, raw events/stderr and sensitive Source content; it is an admission artifact, not product or workflow acceptance.
