...
- Direct meaning:
  - preserve the purpose that Agent development, acceptance and testing completely follow every selected-resource fact, while keeping ordinary tasks materially lighter than Long-Task;
...
- `REQ-DRA-054` — Define complete selected-design conformance once as a shared conditional obligation activated only by a selected implementation handoff; default Contract Conformance and Long-Task Final Gate are mutually exclusive downstream carriers.
- `REQ-DRA-055` — Default Workflow keeps exact ephemeral accounting of covered Source Items, declared verification methods, acceptance blockers, selected targets and conditions; every applicable item reaches the production owner, cold-start journey and an executed current-candidate check whose failure remains attributable. Any unresolved, unmapped, unexecuted, stale or indistinguishable item blocks the complete claim and is reported as a gap.
...
4. It keeps one task-local exact accounting of covered Source Items, declared verification methods, acceptance blockers, selected targets and conditions. Every applicable item reaches the real production owner, cold-start journey and an attributable project check; implementation inspects the first runnable real-entry slice and final-current-candidate Contract Conformance confirms all declared checks ran and passed. Unresolved, unmapped, unexecuted, stale or indistinguishable items block a complete claim and are reported as gaps. Candidate previews and Open Design run success cannot authorize fidelity.
...
- `AC-DRA-029` — Given a selected handoff under the default Workflow, the exact covered Source-item/method/blocker/target/condition sets are routed to production owners, a cold-start journey and attributable project checks on the final candidate. Mutating any selected target, implementation fact or declared check input stales closure; any gap blocks a complete claim without creating a persisted matrix/state/Gate. Accepts `REQ-DRA-054`, `REQ-DRA-055`, `REQ-DRA-060`.
