...
3. Modify only the existing DRA main Skill and existing owner references. Do not create a sixth reference or edit Long-Task behavior. If the required `PROJECT_SPEC.md` change invalidates its already-declared compact semantic input, use only `tools/migrate_long_task_compact_carrier.mjs --sync-source-authority --write` on the existing named Source/Contract pair and accept no population or semantic-owner change.
