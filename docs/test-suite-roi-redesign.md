...
- `tools/test_suite_policy.mjs` is the canonical executable owner for focused/Trust coverage, 32 stable critical-semantic sentinel IDs, the named controlled-CI budget profile and fail-closed Long-Task isolation classes. The Trust file list is derived from the same sentinel records rather than duplicated; the current 32 records derive 24 unique Trust files.
...
- Thirty-two repository-owned records bind a stable semantic ID to one existing test file, required suite(s) and a review rationale; they derive the current 24-file Trust set without a second Trust list or all-test-name manifest. `mechanism-causal-chain-continuity` protects the independent design/Context/current-implementation/owner-change chain. `implementation-freedom-boundary` protects the two-layer `F` decision: the canonical/public carrier remains optional before admission, while the isolated Owner-selected candidate uses its five-predicate positive default, with no development Gate, fixed allocation/count, scheduler, delegation state or proof from worker reports. Both reuse the existing static design-consistency file, so that invariant adds no separate runtime mechanism.
...
- `github-ubuntu-v2` centralizes the current 180/540/1200-second catastrophic ceilings and binds them to `GITHUB_ACTIONS=true` plus `RUNNER_OS=Linux`. It was calibrated on 2026-08-13 from current controlled Ubuntu artifacts: two 61-file default runs took 110.613 and 112.182 seconds, the Trust safe lane took 274.922 seconds before a semantic failure and its three unrun serial files took another 122.978 seconds in the complete run, and the complete 79-file Long-Task population took 925.041 seconds. `github-ubuntu-v1` retains the former 120/240/600-second values for historical comparison but is no longer selected by workflows. Unknown profiles and environment mismatch fail closed; no profile means local diagnostic timing with no cross-machine budget claim. The former inline JSON configuration is retired.
