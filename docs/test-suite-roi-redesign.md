...
- `tools/test_suite_policy.mjs` is the canonical executable owner for focused/Trust coverage, 38 stable critical-semantic sentinel IDs, the named controlled-CI budget profile and fail-closed Long-Task isolation classes. The Trust file list is derived from the same sentinel records rather than duplicated; the current 38 records derive 28 unique Trust files.
