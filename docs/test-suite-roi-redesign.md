...
- `tools/test_suite_policy.mjs` is the canonical executable owner for focused/Trust coverage, 16 stable critical-semantic sentinel IDs, the named controlled-CI budget profile and fail-closed Long-Task isolation classes. The Trust file list is derived from the same sentinel records rather than duplicated.
...
- Sixteen repository-owned records bind a stable semantic ID to one existing test file, required suite(s) and a review rationale. Thirteen records derive the Trust file set and three cover default-suite policy/CI continuity; there is no second Trust list or all-test-name manifest.
