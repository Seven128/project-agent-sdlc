...
The total-cost verifier fails closed. Every formally required Authoring, Runtime, State, Recovery, maintenance, test, process, introduction, adoption and migration metric must have independently attributable verification before `total_roi_supported` can be true. In v3 the workload, run, summary, collection, aggregate and attestation own only raw or recomputable observed-lifecycle facts and expose formal status only as `not_evaluated`; they cannot persist self-attested formal-cost records, evidence admission or support/positivity conclusions. The existing verifier report is the sole formal conclusion owner. Its pre-collection policy now freezes one NCU as CNY 1; active/wait time at CNY 200/CNY 50 per hour only as this delivery's accounting proxies; one ten-delivery cycle; five B/C paired repeatable measurements projected by their median to ten deliveries; maintenance and recovery each measured in five pairs but counted once; introduction, adoption and migration counted once each; and one fixed controlled incident's five-pair median benefit counted once. V3 always collects five A/B/C repeats. A runner-ingested precollection plan freezes and materializes collector, price-document/envelope and redaction inputs; the verifier derives conversions from fixed machine-readable invoice/official-price documents and benefit from raw time/usage, so a packet-authored normalized rate or monetary incident total has no authority. Significant stable margin means benefit is at least 1.25 times the sum of positive incremental costs; each benefit pair subtracts the same representative positive cycle cost; at least 4/5 paired net benefits are positive and sample CV is at most 20%. Cost decreases remain a separate report and do not offset that denominator. Until a bounded independently sourced raw evidence packet and matching precollection lock satisfy that policy, the verifier returns evidence-missing/unsupported; a missing packet has `independent_evidence_admitted=false`, while a structurally admitted packet with missing lock remains supported/positive false. Each materialized workload fixture binds its claimed clean commit/tree to parsed raw HEAD/tree/status command records before and after lifecycle execution and must remain unchanged. Historical v1/v2 artifacts require recollection and cannot prove v3. Machine measurement and independent governance judgment stay separate.
