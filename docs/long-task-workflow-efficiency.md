...
The total-cost scorer fails closed. Every formally required Authoring, Runtime, State, Recovery, maintenance, test, process, introduction, adoption and migration metric must have independently attributable verification before `total_roi_supported` can be true. The current v2 implementation has no independent formal-cost evidence ingestion and admits no self-attested `verified` row, so support remains false until that boundary exists; any missing, unverified or unadmitted category likewise keeps `total_roi_supported=false` and forbids `total_roi_positive=true`. Each materialized workload fixture binds its claimed clean commit/tree to parsed raw HEAD/tree/status command records before and after lifecycle execution and must remain unchanged. Any report-shape change increments its schema version or supplies an explicit tested v1 reader that preserves missing evidence as missing. Machine measurement and independent governance judgment stay separate.
