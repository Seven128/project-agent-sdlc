...
Its mechanism-design objective is **Fine-Grained Semantic Purpose-Fulfillment Efficiency**: preserve the Source/Contract Authority, Authority Lock/Revision, drift observation/localization, repair/revalidation and one current-snapshot Final Gate chain; preserve every independently decidable non-UI Fact and every full-granularity UI/UX Fact/proof obligation; then attain that unchanged complete effect with evidenced high efficiency and high total-cost ROI. Safety, semantic granularity and proof strength are hard constraints rather than costs that may be traded away, and efficiency is a mechanism-admission property rather than a terminal-delivery predicate. Logical Fact and proof-obligation granularity may be finer than physical persistence; unrelated Cartesian axes, derivable repetition and copied shared metadata are not valid long-term cost drivers. A high result has a significant, stable margin after all required costs rather than merely clearing zero; it does not require exhaustive comparison or proof of a global or local optimum.
...
5. then compare every incremental Authoring, Runtime, State, Recovery, maintenance, test, process, introduction, adoption and migration cost with the incremental purpose-fulfillment benefit.
...
Apply a sufficiency stop after the declared-purpose validity floor, relative non-degradation, must-allow controls, Structural Closure Cost Non-Degradation and the applicable measured total-cost thresholds all close. Do not continue expanding the mechanism merely to search for a theoretical optimum. Reopen construction only for a new real counterexample, a repeated material cost hot spot or evidence of significant additional net benefit.
...
## Level 3 Convergence And ROI Reporting

The admitted static/process exact observer remains Level 3. The current subtraction convergence does not alter the Level 4 theorem: promotion still requires complete total-cost ROI evidence and an independent capability audit with no open critical counterexample. A machine-recomputed `observed_lifecycle_*` report is useful diagnostic evidence only; it never substitutes for unverified authoring, maintenance, adoption or migration cost and never carries a `qualified_positive_*`, high-ROI, admission or promotion verdict.

The total-cost verifier fails closed. Every formally required Authoring, Runtime, State, Recovery, maintenance, test, process, introduction, adoption and migration metric must have independently attributable verification before `total_roi_supported` can be true. In real-process v5 with v2 packet/raw-event/scenario/source-manifest schemas and Provider-event v3, workload, run, summary, collection, aggregate and attestation own only raw or recomputable observed-lifecycle facts and expose formal status only as `not_evaluated`; they cannot persist self-attested formal-cost records, evidence admission or support/positivity conclusions. `verify_long_task_real_process_roi.mjs` validates the actual run-set, constructs the immutable artifact index and remains the sole formal conclusion owner; the packet maps expected keys to indexed runner-owned paths and provides no copied raw bytes, digest, role, identity or comparison authority. The pre-collection policy freezes one NCU as CNY 1; active/wait time at CNY 200/CNY 50 per hour only as this delivery's accounting proxies; one ten-delivery cycle; five B/C paired repeatable measurements projected by their median to ten deliveries; maintenance and recovery each measured in five pairs but counted once; introduction, adoption and migration counted once each; and one fixed controlled incident's five-pair median benefit counted once. V5 always collects five A/B/C repeats. The scenario catalog alone freezes all eleven required/forbidden measurement profiles, while a runner-ingested precollection plan materializes capability-only collectors, price-document/envelope, qualified incident and redaction inputs; the verifier derives conversions from fixed machine-readable invoice/official-price documents and benefit from raw time/usage. Significant stable margin means benefit is at least 1.25 times the sum of positive incremental costs; each benefit pair subtracts the same representative positive cycle cost; at least 4/5 paired net benefits are positive and sample CV is at most 20%. Cost decreases remain a separate report and do not offset that denominator. Human time is contemporaneously runner-recorded, compute is whole-process-tree cumulative accounting, storage is runner-owned exact byte-duration or invocation-correlated real billing, and Provider usage is invocation-correlated; unavailable required sources fail collection closed. Each materialized workload fixture binds its claimed clean commit/tree to parsed raw HEAD/tree/status command records before and after lifecycle execution and must remain unchanged. Formal v1 and real-process v1-v4 artifacts require recollection and cannot prove v2/v5. The present controlled-incident Source remains `external_pending` until authorized real or sanitized-real evidence, a complete original-to-sanitized mapping and authorization exist; synthetic controls cannot become formal-positive evidence. Machine measurement, the sole formal verifier, independent audit and project-owner promotion remain separate. Provider HTTPS/parser execution is isolated in the candidate-owned Node worker under the sanitized runtime-TCB-v2 launch envelope, and production source/Git-object identity paths enforce the finite benchmark dependency closure.

The process-runtime subtraction keeps one compiled closure owner. `input_paths` remains Check scope/freshness and is neither broadly role-scanned nor copied. The closure includes the Source-backed root, exact Claim/Counterfactual production carriers and only safe repository-relative independent argv or `--key=value` values that normalize from `cwd` and match a production Binding exact path or pattern; glob-owned and extensionless files are supported. An unmatched safe relative value is not copied. Absolute paths, parent escapes, `file:` URLs and network URLs fail closed unless their semantics are explicitly outside machine closure through the existing external TCB/External Confirmation boundary. Global Checks use internal `<outcome>.<binding>` scoped records, preserving logical identities while deduplicating identical physical paths; authored Bindings and public Contract shape remain unchanged.

R9/R10 proof remains three independent layers: a non-closure unused evidence/verification input is must-allow; an attempted read of a non-closure file cannot be accepted because the isolated runtime does not receive it; and an evidence/verification file explicitly entering the production Binding/argv closure is rejected at Compile for role conflict. Compile owner attribution and stale-Authority protection are also separate: each attack uses a complete committed candidate identity, while a legal near-neighbor Authority followed by the attacked candidate proves Final-Gate freshness/reuse rejection. `active_task_missing` or a fresh Compile rejection cannot substitute for that second chain.

Formal collection also prelocks one fixed scenario catalog plus exact task/gold bytes for the ten cost categories and controlled incident. Each raw event binds a unique output: cost B/C both match the shared gold; incident B differs and C matches. This fixed benchmark type prevents time/usage-only benefit claims without creating a generic registry.
...
The first successful formal Compile creates Authority Lock and unconditionally emits `execution_model_checkpoint.required: true`, `action: change_model_in_host_then_continue`, `resume_token: model checkpoint cleared, continue`, `generic_continue_satisfies: false`, `turn_boundary: end_current_turn`, the four blocked implementation actions, `model_change_owner: host_or_user` and `model_change_observable_by_harness: false`. Before product implementation, the Agent says `After handling the model change, reply exactly: model checkpoint cleared, continue` and ends the current turn; Chinese guidance uses `处理好模型更换后，请仅回复：模型切换卡点解除，继续`. A task-specific model strategy stated earlier never skips the boundary and a generic continuation does not satisfy the package-managed prompt protocol. Harness cannot observe the next host message or verify whether the host model changed, so this remains manual guidance rather than a machine-enforced unlock.

This pause exists because locked Source/Contract/Context/risk/acceptance plus targeted repair and Final Gate make the implementation handoff boundary explicit. It is a host affordance, not proof. Harness does not switch the parent model, persist acknowledgement/model-routing state or repeat the checkpoint after later revisions; those Compile results return `required: false`. Once resumed, the parent performs an internal Delegation Suitability judgment and uses the package-managed fixed `long_task_implementation` profile only when the host can explicitly select that exact custom agent and independent bounded benefit exceeds coordination cost. Multiple disjoint exact instances are allowed; high-coupling work stays with the parent. A generic substitute is forbidden, and unsupported selection falls back to parent execution without retry. The static, stateless, non-authoritative profile cannot satisfy the checkpoint or acceptance and is never required.

After resume, the current parent Goal retains Source, Contract, Authority, architecture, Context, integration, current-candidate checks, formal verification, Final Gate and closure while implementation, targeted repair and Final Gate continue. Small implementation-level plans, reordered steps and repair hypotheses are ordinary internal execution state; they do not become a second plan or authority.
...
npm run test:long-task:level4
```

The affected selector maps known hot spots to focused regression tests, widens unmapped Long-Task runtime changes to the core Trust Boundary Gate, and separately marks the Level-4 lane required for direct Level-4 owners, every member of its frozen implementation closure, reviewed test/package/CI routing dependencies and unknown paths. Shared fixtures and package/dependency changes still widen to complete suites. Dirty local discovery uses only current working-tree paths; clean local discovery uses `HEAD^`; explicit and CI bases are exact. It builds at most once per invocation when a build is required.

The 18-file core Trust Boundary Gate is the middle-cost frozen-candidate check for independent high-impact Level-3 authority, freshness, forged-evidence, Final Gate/Stop/close, Hook/profile and platform-boundary regressions plus a lightweight no-self-promotion control. The disjoint seven-file Level-4 lane retains the real-process workload and complete formal promotion chain. Pull requests run default plus core Trust, adding Level-4 and its Windows positive path only when the exact affected plan requires them; `main`, manual, publish and release retain the exhaustive package and Windows gates. Inside a complete Long-Task run, the same registry projects core Trust, Level-4 and remainder exactly once; logical lanes above the current 16-file process bound are split into ordered non-overlapping batches. Files are not rerun as a second aggregate, and semantic failures are never retried.
