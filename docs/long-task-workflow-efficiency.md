...
This pause exists because locked Source/Contract/Context/risk/acceptance plus targeted repair and Final Gate materially reduce the risk of choosing a lower-cost execution model. It is an execution-cost mechanism, not proof. Harness does not switch the parent model, persist acknowledgement/model-routing state or repeat the checkpoint after later revisions; those Compile results return `required: false`. After the choice, Codex may optionally use the package-managed fixed `long_task_implementation` (`gpt-5.6-luna`, `max`) profile for bounded implementation/repair. That static, stateless, non-authoritative profile cannot satisfy the checkpoint or acceptance and is never required; parent execution is the fallback.
