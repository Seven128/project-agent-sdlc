...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.8.0 and is published to npm.
```
...
```sh
node tools/github_release_publish.mjs --version 0.8.0
```
...
2. Choose tag `v0.8.0`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.8.0`.
4. Use title `Project Tiny Context Harness 0.8.0`.
...
- Do not retarget `v0.8.0` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.8.0` remains live and installable.
