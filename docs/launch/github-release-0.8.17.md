...
- Makes unresolved substrate observations target-wide: every `decision_required` or `unavailable` observation must be carried by exact blockers across every material family × condition cell; blocker and marked-Source projection observation-ref sets must match, bad/duplicate refs and zero-family vacuity fail, and candidate cells require observed component roots.
- Proves the complete production path universe against observed roots: every `file`/`path_glob` target and carrier, every `verified` carrier and every planned non-empty carrier must be a proven repository-pattern subset. One inside path cannot hide an outside or unknown carrier.
- Keeps implementation choice on the existing authority path: DRA enumerates allowed candidates, Default Workflow selects in Architecture Deliberation, and an active Long-Task derives exactly one allowed realization per candidate-bearing feasibility cell from existing component bindings, rejects missing/ambiguous selection and unattributed bindings, and requires exact `path#source-item-key` Claims for selected planned owners and required realizations. Standalone Contract validation still requires component refs; full activation admits an empty set only for an all-blocker-only surface with valid route and blocker closure. No new Contract field, Claim type or Final Gate is added.
- Makes every current feasibility blocker a real Long-Task completion boundary: it compiles only as `decision_required` or an existing target-blocking External Confirmation covering an affected target Claim. Full activation excludes only those externally blocked design Assertions from machine execution, preserves the original Contract and sole Final Gate, and finishes at `blocked_external` rather than `machine_accepted`. Resolving a blocker requires revising the technical Source and feasibility input rather than passing the stale blocker.
- Distinguishes canonical visual timing from technical duration: motion-context and ambiguous time literals remain forbidden in feasibility prose, while explicit build/compile/test/startup/latency/timeout/network/benchmark/runtime-cost durations are allowed.
