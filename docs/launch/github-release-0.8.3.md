...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.8.3 and is published to npm.
```

Title:

```text
Project Tiny Context Harness 0.8.3
```
...
````markdown
Project Tiny Context Harness 0.8.3 is the current public release line under the renamed npm package:
...
- Publishes `project-tiny-context-harness@0.8.3` with the synchronized package assets and CLI build.
- Keeps the install path on the renamed package: `project-tiny-context-harness`.
- Strengthens selected-design handoffs with complete, typed fact inventories instead of treating product Control granularity as the observable-fact ceiling.
- Requires every acquired resource to close as material-with-facts or supporting-only, with exact fact, evidence, Source and verification-method bindings.
- Carries exact design fact references through Long-Task compilation and runtime evidence so omitted or substituted facts fail closed.
- Adds full-target layout and pixel closure for exact targets while keeping partial resources scoped as constraints or unresolved blockers.
- Fixes complete verifier execution on Windows and expands the affected-test routing for the new fact-completeness owners.
...
```sh
node tools/github_release_publish.mjs --version 0.8.3
```
...
2. Choose tag `v0.8.3`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.8.3`.
4. Use title `Project Tiny Context Harness 0.8.3`.
...
- Do not retarget `v0.8.3` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.8.3` remains live and installable.
