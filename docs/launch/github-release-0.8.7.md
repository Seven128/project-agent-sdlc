...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.8.7 and is published to npm.
```

Title:

```text
Project Tiny Context Harness 0.8.7
```
...
````markdown
Project Tiny Context Harness 0.8.7 is the current public release line under the renamed npm package:
...
- Publishes `project-tiny-context-harness@0.8.7` with the synchronized package assets and CLI build.
...
```sh
node tools/github_release_publish.mjs --version 0.8.7
```
...
2. Choose tag `v0.8.7`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.8.7`.
4. Use title `Project Tiny Context Harness 0.8.7`.
...
- Do not retarget `v0.8.7` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.8.7` remains live and installable.
