...
- UI surface binding uses the same cost shape: one grouped declaration per production surface/product target rather than per Control field; reuse existing route/component Bindings and root Checks; run the real-entry Check after each runnable vertical slice; and require `design_conformance` only for selected exact/constraint targets. No universal screenshot matrix or second visual audit run is imposed.
