...
Every Long-Task change starts from the controlling design purpose. Before cost is considered, prove `Coverage_new ⊇ Coverage_old`, `FalseNegative_new ⊆ FalseNegative_old`, Authority and fail-closed non-bypassability, and complete-current-final-snapshot proof. If that proof is unavailable, keep the formal acceptance path unchanged and place any experiment outside it. Only after this first gate, account for total cost by explicitly including the cost of introducing and migrating the change in the subsequent ROI judgment. If the change alters mechanism semantics, an invariant, an authority/proof boundary or runtime behavior, change the mechanism and its verification. Otherwise change only the owning point; do not turn a local correction into a new mechanism.
...
`Context Delta` remains live after Authority Lock. When implementation or repair discovers a durable fact, the current Goal updates the owning Context rather than preserving a known stale fact until the end. In referenced mode, core Context, explicit `context_refs`, verification/deployment Context and every selected requirement-bearing role are **Controlling Context**; changes require Authority Revision, invalidate affected evidence and require a user decision only if mapped Claims/targets/proof obligations or another decision boundary change. Only graph-derived, non-explicit `implementation-index` and `archive` files are **Supporting Context**; a supporting-only content revision may additionally preserve otherwise-fresh targeted Progress. Full snapshot mode and explicit references treat every selected file as controlling. Final Gate always recompiles and records the complete current Context snapshot.
...
New authoring uses Compact V2 YAML. The parser fills only deterministic defaults. Goal, target profile and required target refs, execution targets/root entrypoints, Stages, Source/Source Claims, Context, vertical Outcomes, applicability profiles and refs, required success/degradation paths, owners/paths, Requirements, complete Control field/relation closure, obligations, proof surfaces, journey roles, keyed Given/When scenarios, Evidence Capabilities, runner targets, verification inputs, one-Claim Assertions, semantic Counterfactual/liveness witnesses, risk facts, forbidden shortcuts and typed external confirmations remain explicit. Compact and expanded forms normalize to the same Contract object, hashes, risk, Claim Coverage and compiled identity.

Compile is deterministic, static and model-free. It calls the shared activation-safety kernel in fail-fast mode, so skipping Preflight bypasses no safety rule. The kernel strictly parses YAML, validates schema, Stage closure/dependencies and vertical Outcome placement, classifies every Source byte, inventories Source Items, validates canonical targets, complete Control field/relation closure, applicability-to-target/journey/Given/When equality, required-target/root/runner bindings, scenario and success/degradation separation, Evidence Capability adequacy, typed external impact, bounded Product Conformance, Context refs, safe paths, owners, Bindings, runners and verification inputs, rejects unsafe authority/proof files, compiles and requires all-of Claim × applicability × proof-surface Coverage, validates Source-to-Claim/Acceptance/Result/Global/Risk/external/decision mappings, enforces explicit one-Claim Assertions and compatible adapters, computes risk, enforces semantic Counterfactual/liveness, Population/security/environment/recovery proof and freezes Source, Contract, selected Context, verifier, runner, workdir and repository identity.
...
- semantic replacement Counterfactuals plus independent target-liveness witnesses for every behavioral Claim-bearing Assertion;
...
- typed, Assertion-bound Evidence Capability records for interaction, state change, cross-surface consistency, durable readback, boundary invocation, external effect, failure injection, rendered UI, selected-target design conformance, target runtime and input variation;
...
A selected exact/constraint design target additionally names immutable verification inputs, condition keys, bound Control Claims, one root-entry conformance Check/Assertion and current actual/comparison artifact paths. That Assertion requires `design_conformance`, `interaction_trace` and `target_runtime`; the typed conformance record must match the compiled target/conditions/artifact paths and both artifacts must be present in the current Check result. `visual_render` proves only render existence, while file hash/count/registry validation is `design_resource_integrity`; neither proves `design_implementation_conformance`. Each surface binding explicitly inventories declared design-acceptance blockers; every entry resolves through a target-local machine Claim or a target-blocking External Confirmation with matching impact. An empty array means none were declared, not that a known blocker was waived; removing one requires revised Source/Contract authority. The Harness trusts the frozen project verifier's comparison result but does not infer undeclared blockers from arbitrary external formats.
