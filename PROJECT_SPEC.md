...
One design-specific Long-Task objective is that Agent implementation, acceptance and testing fully conform to every material UI/UX fact that selected design resources explicitly express within their declared scope and conditions. Direct visual inspection is a lossy boundary: layout, control styling, state, interaction, motion, adaptation/input, accessibility and asset meaning can be overlooked, flattened or reinterpreted before it becomes Contract authority. This purpose is bounded by expression: the workflow never infers an unshown interaction or proves that the user omitted nothing. Missing meaning is authored, explicitly excluded/not-applicable, or remains `decision_required`/`unavailable` and blocking.

The transformation has three complementary owners. First, `design-resource-authoring` uses Open Design's demonstrated ability to emit implementation-readable HTML/CSS/JS plus specifications, tokens and assets, but turns that capability into an invariant: Web/App handoff requires a canonical machine-readable entry, complete retrieved dependency set, immutable bytes/digests and stable addressable facts; a PNG may be a derived baseline but not the sole implementation source. Before `ready`, authoring exercises every declared verification method on that immutable source under its claimed conditions and cross-checks repeated facts across code/specs/tokens/assets; a mismatch is refined or remains `decision_required`/`unavailable` and blocking. This is source QA, not production acceptance. Second, one provider-neutral `design-resource-handoff-v1` remains the residual scope/applicability/semantic adapter. Exact code-expressible values stay in canonical resources; the handoff indexes them through typed locally resolved locators, expands every applicable subject × target × condition × eight-dimension cell and supplies exclusions, product/data/permission/recovery meaning, unresolved items, blockers, Source Items and verification methods.

Third, downstream authority makes consumption and proof explicit: immutable resource plus residual handoff → Context-reachable target → Source Claims and applicable Controls/`surface_bindings` → production route/component owner and cold-start journey → root conformance Assertion plus one independent Assertion per declared verification method → typed current-execution evidence → current-snapshot Contract Conformance or the sole Long-Task Final Gate. Each blocker retains the same Source Item/method lineage into a target-local machine Claim or target-blocking External Confirmation. Provider success, a screenshot, resource hash, locator preflight or handoff integrity proves only acquisition/addressability/integrity and cannot substitute for production conformance or verifier mutation sensitivity. Missing entry/dependency closure, unresolvable locator, target/condition cell, method binding, blocker lineage, resource or decision fails closed. The chain adds no provider-specific schema, registry, Claim kind, second Authority, second Gate or acceptance lifecycle.
...
The Skill then creates one project-native marked Markdown Source with exactly one strict residual `design-resource-handoff-v1` block and runs `ty-context design-resource preflight`. Exact code-expressible values remain in canonical resources; the adapter records residual scope/provenance/resources/conditions/subject applicability/targets/typed evidence, closes every applicable cell across all eight UI/UX dimensions, binds Source Items and verification methods, and refuses `ready` for unresolvable, partial, unresolved or stale input. Exploration remains schema-free. The default Workflow reruns the same preflight before UI Authority Closure. Long-Task reruns it during Preflight/Compile and requires target/files/conditions equality, Source Claims in the root Assertion, one independent Assertion per verification method and exact blocker lineage. The adapter proves semantic input completeness, addressability and resource integrity only; mutation-sensitive project Checks and default Contract Conformance or Long-Task Final Gate prove current production implementation.
