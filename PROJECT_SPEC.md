...
2. **Workflow Contract** defines the lightweight default loop: core/default Context, manifest routing plus one bounded Context search, one externally observable Architecture Deliberation, one `Context Delta`, a conditional Design Authority Check for material production UI, platform-internal planning, implementation, project verification, Contract Conformance including Architecture Conformance and selected-design closure when a selected implementation handoff exists, and a separate Context drift check.
...
The highest Long-Task objective is to prevent false completion inside declared authority. Its irreducible upstream trust boundary is that the authoritative delivery Source is complete, accurate and fine-grained enough to state every intended requirement and acceptance criterion, including Control and cross-Control/system relations; the workflow cannot discover a requirement the user never expressed. Once that Source exists, meaning-preserving Source-to-Contract-to-observer projection, applicability coverage, falsifiable oracle coverage, fail-closed aggregation, protected-input freshness and complete Final Gate execution are workflow proof obligations rather than assumed prerequisites. Under that boundary, no declared Plan Item or AC may be accepted or reported as complete until it is actually satisfied.
...
One shared conditional delivery objective is that Agent implementation, acceptance and testing fully conform to every material UI/UX fact that selected design resources explicitly express within their declared scope and conditions. It activates only for a selected implementation handoff. Direct visual inspection is a lossy boundary: layout, control styling, state, interaction, motion, adaptation/input, accessibility and asset meaning can be overlooked, flattened or reinterpreted before proof. This purpose is bounded by expression: neither workflow infers an unshown interaction or proves that the user omitted nothing. Missing meaning is authored, explicitly excluded/not-applicable, or remains `decision_required`/`unavailable` and blocking.

The transformation has three complementary owners. First, `design-resource-authoring` uses Open Design's demonstrated ability to emit implementation-readable HTML/CSS/JS plus specifications, tokens and assets, but turns that capability into an invariant: Web/App handoff requires a canonical machine-readable entry, complete retrieved dependency set, immutable bytes/digests and stable addressable facts; a PNG may be a derived baseline but not the sole implementation source. Before `ready`, authoring exercises every declared verification method on that immutable source under its claimed conditions and cross-checks repeated facts across code/specs/tokens/assets; a mismatch is refined or remains `decision_required`/`unavailable` and blocking. This is source QA, not production acceptance. Second, one provider-neutral residual handoff, `design-resource-handoff-v1`, remains the scope/applicability/semantic adapter. Exact code-expressible values stay in canonical resources; the handoff indexes them through typed locally resolved locators, expands every applicable subject × target × condition × eight-dimension cell and supplies exclusions, product/data/permission/recovery meaning, unresolved items, blockers, Source Items and verification methods.

Third, downstream authority conserves the exact covered Source-item, verification-method, acceptance-blocker, target and condition sets into one production-proof carrier. The default Workflow keeps an ephemeral mapping to production owners, cold-start journeys and independently attributable project checks, then closes it through final-current-candidate Contract Conformance. Long-Task projects the same sets into existing Source Claims, explicit target/journey/Given/When applicability cells, Controls/`surface_bindings`, root and method-specific Assertions, typed current-execution evidence and its sole Final Gate. The carriers are mutually exclusive, so Long-Task never also runs the default closure. Each blocker retains the same Source Item/method/required-capability lineage into capability-qualified target-local machine proof or a target-blocking External Confirmation. Provider success, a screenshot, resource hash, locator preflight or handoff integrity proves only acquisition/addressability/integrity and cannot substitute for production conformance or verifier mutation sensitivity. Missing entry/dependency closure, unresolvable locator, applicability cell, mapping, method proof, blocker lineage, capability, resource or decision fails closed. The default reports checked scope and gaps instead of a complete claim; it persists no matrix, Claim set, state or Gate. The chain adds no provider-specific schema, registry, Claim kind, second Authority, second Gate or acceptance lifecycle.
...
- Architecture remains in existing Source-backed Technical Obligations, Constraints/Forbidden Shortcuts, owner/path/Binding boundaries and executable Checks. Source explicitly classifies at least one architecture obligation and Final Gate reruns its Claim proof; no architecture Contract field, second Gate/state or generic analyzer is added.
...
The efficiency objective is the lowest practical total workflow cost that preserves this false-completion interception strength. Admission is ordered. First prove `Coverage_new ⊇ Coverage_old`, `FalseNegative_new ⊆ FalseNegative_old`, and that Authority, fail-closed behavior and complete-current-final-snapshot proof remain non-bypassable. Cost savings never compensate for weaker drift detection; an unproven optimization stays outside the formal acceptance path. Only then compare all incremental Authoring, Runtime, State, Recovery, model, maintenance, test, process, introduction and migration costs. A candidate has positive ROI when its incremental purpose-fulfillment benefit exceeds all of those incremental costs; positive ROI places it in the consideration set and does not automatically adopt it. Prefer one cohesive implementation when partial changes would leave semantic seams or cost more through repeated migration and maintenance. Efficiency never authorizes uncertain acceptance: when the evidence boundary is unclear, the workflow still fails closed.
...
Let `P` be the controlling accepted-terminal-state purpose above, `K` the enabling logic that authoritative meaning must be sufficiently precise, accurate, applicable and observable for drift to be defined, `R1` the meaning-capture/architecture responsibility, `R2` the fail-closed observation/repair/final-snapshot responsibility, `B` the explicit Source-completeness, semantic-binding, observability and trusted-computing-boundary conditions, and `I_current` the actual current repository implementation. A valid current-version claim requires:
...
Cross-version admission is lexicographic. First prove `Coverage_new ⊇ Coverage_old`, `FalseNegative_new ⊆ FalseNegative_old` and preserve non-bypassable Authority, fail-closed behavior and complete-current-final-snapshot proof. Only after that hard gate may a candidate be considered when its incremental anti-degradation or purpose-fulfillment benefit exceeds every incremental Authoring, Runtime, State, Recovery, maintenance, test, process, introduction and migration cost. Positive net ROI permits consideration, not automatic adoption; lower cost cannot compensate for weaker interception.

The protected design invariants are `P`, `K`, the two jointly necessary responsibilities `R1`/`R2`, and boundary `B`. Ordinary requirement, implementation or optimization work may strengthen them or replace concrete implementation details without changing their meaning. Weakening, reinterpreting or replacing a protected invariant requires an explicit project-owner design-purpose decision recorded in this specification and owning Context. That decision must name the old and new purpose/implication/boundary, the coverage and false-negative effect, compatibility and migration consequences, and replacement proof. Agent inference, simultaneous edits to prose/code/tests or a cost argument alone cannot silently authorize the change.

The current construction uses one canonical causal explanation, Context discovery triggers, an implementation/proof index, executable semantic counterexamples and current-snapshot tests, stable critical-sentinel continuity, fail-safe affected-to-Trust-to-complete routing, managed-source/build/Context parity and existing CI/release gates. These layers catch independently different drift paths and remain ordinary project evidence; none becomes acceptance authority. They protect known or declarable invariants and make normal or accidental weakening fail review or verification. No repository-local rule can be cryptographically immutable against an authorized maintainer who deliberately weakens the specification, implementation and tests together, and no test can recover an omitted or unobservable requirement; those are explicit governance and theorem boundaries rather than hidden guarantees.
...
Before Long-Task activation, every Material Source Item receives a non-rendering, meaning-preserving marker in the original Source without rewriting its text. Every non-empty Source line must be owned by exactly one Material Item, an explicit keyed `ty-source-background` block with a non-authoritative reason, or the one formally parsed `design-resource-handoff-v1` block. Unclassified text, malformed/overlapping ownership and invalid formal blocks fail closed. A background block may hold only genuine structure, navigation or provenance; it cannot be used to hide requirement, acceptance, technical, risk or decision meaning. Contract authoring may add only:
...
The shared design-resource input adapter satisfies the same admission rule one boundary earlier. Direct visual-only consumption is not a reliable Contract interface: a prose summary or generic `condition_keys` can silently omit or flatten a category before implementation begins. Open Design can already emit source-rich implementation resources, but prior V1 treated that as an authoring promise, accepted opaque locator strings and closed only `subject × dimension`. Executable counterexamples showed that nonexistent locators, a new uncovered condition and a second almost-uncovered target all still returned `ready`; downstream verification methods were not consumed and blocker keys could bind unrelated proof.

The strengthened adapter converts those distinct false-green paths into invariants: Web/App source profiles freeze a canonical entry and exact dependency set; typed locators resolve locally; coverage expands each applicable subject × target × condition × dimension cell and requires same-cell evidence; every method maps to an independent existing Assertion; every blocker retains Source Item/method lineage. Exploration pays no schema cost, non-Web `reference` resources remain valid, and selected handoffs pay one compact residual index rather than duplicating resource values or creating one artifact per control. Figma, Penpot and OpenPencil remain optional upstream choices for real collaboration/editability needs, but converting a complete Open Design source into a second representation does not close another workflow-enforcement gap. Link-only/prompt-only approaches cannot be validated, manual duplication increases drift, and a provider-specific pack/schema/registry/Gate would duplicate ownership and add migration/runtime cost. The admitted extension therefore has positive omission/sensitivity ROI after the non-degradation gate without adding Design Authority, workflow state, Receipt, second Authority or second Gate.
...
When a selected implementation handoff exists, L0 remains lightweight but not lossy. It keeps one task-local exact accounting of covered Source Items, declared verification methods, blockers, targets and conditions; routes each item through the production route/component owner and cold-start real-user journey; and requires attributable project checks on the final current candidate. One check may cover several methods only when each method and fact can fail distinctly. Any unresolved, unmapped, unexecuted, stale or indistinguishable item blocks a complete selected-design-conformance claim. No Delivery Contract, persisted map, evidence ledger, readiness state or extra Gate is created.
...
For Web/App implementation handoff, the Skill commissions an implementation-level Open Design output and retrieves the canonical entry plus complete dependency set without truncation. It freezes exact bytes/digests and requires machine-resolvable stable locators for declared facts. PNG-only, mutable-link, metadata-only and partial acquisition cannot authorize implementation fidelity. Figma, Penpot, OpenPencil and other native design platforms remain optional upstream producers when their collaboration/editability value is independently required; all feed the same provider-neutral adapter.

The Skill then creates one project-native marked Markdown Source with exactly one strict residual `design-resource-handoff-v1` block and runs `ty-context design-resource preflight`. Exact code-expressible values remain in canonical resources; the adapter records residual scope/provenance/resources/conditions/subject applicability/targets/typed evidence, closes every applicable cell across all eight UI/UX dimensions, binds Source Items and verification methods, and refuses `ready` for unresolvable, partial, unresolved or stale input. Exploration remains schema-free. The default Workflow reruns the same preflight before UI Authority Closure and conserves exact handoff sets into task-local production checks and final-current-candidate Contract Conformance. Long-Task reruns it during Preflight/Compile and projects the same sets into target/files/conditions equality, Source Claims, root and method-specific Assertions, blocker lineage and Final Gate. Only one downstream carrier runs. The adapter proves semantic input completeness, addressability and resource integrity only; mutation-sensitive project Checks and the owning carrier prove current production implementation.
...
`context_uiux_design` owns downstream portable design-authority judgment and repairs/adopts missing or conflicting durable Design Authority without creating a visual workflow. Standalone low/high-fidelity, candidate, prototype or conditional Figma-handoff commissioning routes to `/design-resource-authoring`; `context_uiux_design` consumes selected Source only during a separately authorized development workflow, performs UI Authority Closure and applies deterministic Design Source Projection. It writes durable surface/flow/Screen/Control/state meaning to existing Product Surface or Screen/interaction Context, durable visual-system/token/motion-policy/rationale meaning to `DESIGN.md`, and repeatable proof routes to verification Context while exact target facts and delivery-local handoff/proof meaning stay outside Context. Each adopted target receives exactly one canonical record in `DESIGN.md` for project/system/component-family scope or the owning Screen Contract for one-screen/interaction scope; other layers retain a stable-key owner/anchor pointer only. For material design-system, redesign or high-fidelity work the Skill may use task-local risk-proportional coverage reasoning across production surface/component, viewport, theme/mode, interaction/state, content stress and accessibility/motion conditions to prioritize authoring and feedback. It may not prune any declared/applicable combination or replace it with risk-only/pairwise sampling unless authoritative Source narrows the scope or project-owned proof establishes equivalence. It creates no competing file, Cartesian matrix, Context role or authority and cannot infer completeness beyond declared coverage.

`context_development_engineer` carries that intent into implementation by confirming Design Authority readiness, locating the production token/component/route owners, following every selected target/condition through a cold-start real-user path to rendered/interactive comparison evidence, preventing undeclared raw-value, circular self-baseline or detached-demo bypasses, and checking each independently runnable vertical slice before dependent UI work expands. For a selected handoff it accounts for the exact covered Source-item/method/blocker/target/condition sets and refuses a complete claim until all are attributable to passing final-candidate production checks. Static kits, mocks and marketing specimens may inform review but cannot substitute for production components or real product routes. Resource-integrity and implementation-conformance checks are named separately. Both Skills report only combinations actually checked; omitted combinations remain unproven.
...
Visual Long-Task delivery is an authoring/evidence specialization, not a new mechanism or second workflow. The validated handoff Markdown enters `source_paths` as ordinary marked Source; acceptance-affecting selected targets, the handoff, token sources and fixed resource snapshots enter `verification_inputs`; production owners enter `input_paths`/Technical Bindings; generated implementation renders/diffs/reports remain `artifact_globs`. Before Compile, material production UI runs the shared handoff preflight, performs UI Authority Closure, traverses owning Context/`DESIGN.md` stable keys and opens each affected selected exact target/constraint, confirming immutable adopted locator/digest, conditions and editable upstream owner/locator/update route. Missing/stale/unreadable/conflicting resources remain `decision_required`; updates produce a new immutable version instead of overwriting the baseline. The Control projection preserves each applicable surface, region/location, type/label, user task, visibility/availability, trigger/input/validation/default, interaction/navigation, loading/empty/success/failure/recovery/permission/feedback and accessibility field as an independent Source-backed Claim and protected product semantic.
